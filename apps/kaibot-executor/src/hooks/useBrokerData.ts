        ...
        (async () => {
          try {
            const [positionsRes, balancesRes] = await Promise.all([
              apiFetch(`/api/exchanges/v2/positions/${session.exchangeName}`, {
                headers: { "x-user-id": "default" },
              }),
              apiFetch(`/api/exchanges/v2/balances/${session.exchangeName}`, {
                headers: { "x-user-id": "default" },
              }),
            ]);
            if (positionsRes.ok) {
            ...
            if (balancesRes.ok) {
              balances.set(session.exchangeName, await balancesRes.json());
            }
          } catch (error) {
            console.error(
