  ...
  return (
    ...
    >
      <div className="flex justify-between items-center gap-4 px-3 py-2" data-tauri-drag-region>
        ...
        {isViewer && (
          <Badge variant="outline" className="font-mono text-[10px] uppercase tracking-wider" title="This account can read everything and change nothing">
            View only
          </Badge>
        )}

        <div className="flex items-center gap-0.5" data-tauri-drag-region={undefined}>
          {/* API Connection Indicator (the admin owns the pairing) */}
          {!isViewer && !apiConnection.isLoading && !apiConnection.isConfigured && (
            <Button
              variant="ghost"
              size="icon-sm"
