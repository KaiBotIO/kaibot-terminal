...
import { opsApi, type PositionLineage } from '@/lib/ops-api';
import { defaultSymbolFor, isVenueDefault } from '@/lib/default-symbol';
import { apiFetch } from '@/lib/api';
import { equalSizes, weightedSizes } from '@/lib/rung-presets';
import { notionalOf, orderNotional } from '@/lib/notional';
...
export function ManualTradePanel() {
  ...
  // Which lineage (bot vs manual) holds each live position — from the book,
  // not a symbol match: a bot's root ('MES') never equals the position's dated
  // contract ('MESU26'), which used to badge every bot position MANUAL.
  const { data: lineageData } = usePolledResource(() => opsApi.positionLineage(), { intervalMs: 8000 });
  const lineages: PositionLineage[] = lineageData?.lineages ?? [];
  ...
  // Connection (exchange + label) the order goes to. `exchange` stays the bare
  // venue name for every API body; the label only scopes the account list, and
  // the picked accountId ('acct2/btc') carries the connection from there on.
  const [connectionKey, setConnectionKey] = useState('');
  const selectedConnection = connected.find((s) => sessionKey(s) === connectionKey);
  const exchange = selectedConnection?.exchangeName ?? '';
  useEffect(() => {
    if (!connectionKey && connected.length > 0) setConnectionKey(sessionKey(connected[0]));
  }, [connected, connectionKey]);

  // Broker accounts on the selected venue. Multi-account venues (TradeStation)
  // route orders by AccountID — the backend refuses to pick one silently, so
  // the panel must send an explicit choice.
  const [accountId, setAccountId] = useState('');
  const [venueAccounts, setVenueAccounts] = useState<Array<{ accountId: string; name?: string }>>([]);
  useEffect(() => {
    setAccountId('');
    setVenueAccounts([]);
    if (!exchange) return;
    let cancelled = false;
    const q = selectedConnection ? accountQuery(selectedConnection) : '';
    apiFetch(`/api/exchanges/v2/accounts/${exchange}${q}`, { headers: { 'x-user-id': 'default' } })
  ...
  // The symbol field follows the venue: an empty field, or one still holding a
  // previous venue's default, takes the picked exchange's default.
  useEffect(() => {
    if (!exchange) return;
    setSymbol((cur) => (cur === '' || isVenueDefault(cur) ? defaultSymbolFor(exchange) : cur));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [exchange]);
  ...
  // Badge label: the book's lineage first (exact exchange/account/symbol), the
  // running-bot match as fallback, else manual.
  const lineageFor = (p: Position): PositionLineage | undefined =>
    lineages.find(
      (l) =>
        l.exchange === p.exchange &&
        l.symbol.toUpperCase() === p.symbol.toUpperCase() &&
        (!l.accountId || !p.accountId || l.accountId === p.accountId),
    );
  const managedLabel = (p: Position): string | null => {
    const lineage = lineageFor(p);
    if (lineage?.source === 'bot') return lineage.botName || 'Bot';
    const bot = botFor(p);
    return bot ? bot.botName || bot.strategyName || 'Bot' : null;
  };
  ...
  const close = async (p: Position) => {
    ...
    try {
      const r = await manualTradeApi.close({
        ...
        symbol: p.symbol,
        // The position knows its own broker account — close on that account,
  ...
  const placeSummary = (): ConfirmSummaryItem[] => {
    ...
    // USD-mode: the entered size IS the notional. Native-mode: notional ≈ qty ×
    // price × contract multiplier (1 MGC at 4.500 is $45.000, not $4.500).
    if (sizeUnit === 'usd') items.push({ label: 'Notional', value: `≈ $${fmt(qty)}` });
    else if (px != null && px > 0)
      items.push({
        label: 'Notional',
        value: `≈ $${fmt(orderNotional({ exchange, symbol: sym, qty, price: px }))}`,
      });
    return items;
  };
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  placeholder={defaultSymbolFor(exchange) || 'Symbol'}
                  value={symbol}
                ...
                >
                  ...
                  {venueAccounts.map((a) => (
                    <option key={a.accountId} value={a.accountId}>
                      {a.accountId}
                      {a.name && a.name !== a.accountId ? ` — ${a.name}` : ''}
                    </option>
                  ))}
                </select>
              </div>
