...
export interface ApiConnectionSnapshot {
  ...
  apiUrl: string | null;
  error: string | null;
  // A viewer may not read the pairing (403); the surfaces that warn about a
  // missing key stay quiet instead of claiming there is none.
  restricted: boolean;
}
...
export function refreshApiConnection(): Promise<void> {
  ...
  inFlight = (async () => {
    try {
      ...
      if (res.status === 403) {
        publish({ isLoading: false, isConfigured: false, apiUrl: null, error: null, restricted: true });
        return;
      }
      if (!res.ok) {
...
export function resetApiConnectionStore() {
  snapshot = { isLoading: true, isConfigured: false, apiUrl: null, error: null, restricted: false };
  inFlight = null;
}
