...
export interface TerminalSrcParams {
  ...
  embed?: boolean;
  /**
   * This shell's origin. Studio only accepts bridge messages (own trades,
   * positions) from it, so it travels with the embed URL. Ignored without embed.
   */
  host?: string | null;
}

// Query param carrying the shell origin. Mirrored in
// apps/frontend/src/lib/embed-mode.ts.
export const EMBED_HOST_QUERY_PARAM = 'host';
...
export function buildTerminalSrc(
  ...
  if (params.embed) {
    ...
    if (host) url.searchParams.set(EMBED_HOST_QUERY_PARAM, host);
  }
  if (symbol) {
