...
async function fetchExchangeDetails(exchangeName: string, accountKey: string | null): Promise<ExchangeDetails> {
  const q = `?account=${encodeURIComponent(accountKey ?? 'default')}`
  const response = await apiFetch(`/api/exchanges/v2/${exchangeName}/details${q}`, {
...
export default function ExchangeDetailPage() {
  // Key by exchange + connection so navigating between them resets the polled state.
  const { exchangeName } = useParams<{ exchangeName: string }>()
  const [search] = useSearchParams()
  // '?account=default' is the default connection: no key, plain title.
  const raw = search.get('account')
  const accountKey = raw && raw !== 'default' ? raw : null
  return (
    <ExchangeDetailInner
      key={`${exchangeName}:${accountKey ?? ''}`}
...
function ExchangeDetailInner({ exchangeName, accountKey }: { exchangeName: string; accountKey: string | null }) {
  const navigate = useNavigate()
  const accountQ = `?account=${encodeURIComponent(accountKey ?? 'default')}`
  ...
  const { data: details, error, isStale, isLoading, lastUpdated, refresh } = usePolledResource(
    () => fetchExchangeDetails(exchangeName, accountKey),
    { intervalMs: 30000 },
  )
  ...
  // Keyed on (account, symbol): the same contract can be healthy on one account
  // while another account carries the incident.
  const mismatchedPairs = useMemo(
    () => new Set((recon?.mismatched ?? []).map((r) => `${r.account_id}:${r.symbol}`)),
    [recon],
  )
  ...
  const handleRefresh = async () => {
    setRefreshing(true)
    try {
      await apiFetch(`/api/exchanges/v2/${exchangeName}/refresh${accountQ}`, {
        method: 'POST',
        headers: { 'x-user-id': 'default' },
      })
    } catch {
  ...
  return (
    <div className="flex flex-col">
      ...
      <PageHeader
        title={accountKey ? `${exchangeName} · ${accountKey}` : `${exchangeName} exchange`}
        meta={
...
function PositionsTab({
  ...
  /** "<accountId>:<symbol>" keys with an unsettled reconciliation delta. */
  mismatchedPairs: Set<string>
}) {
          ...
          columns={[
            {
              ...
              cell: (position) => (
                <span className="inline-flex items-center gap-1 font-mono font-medium text-[var(--kb-teal)]">
                  {position.symbol}
                  {mismatchedPairs.has(`${position.accountId}:${position.symbol}`) && (
                    <Badge
                      variant="error"
