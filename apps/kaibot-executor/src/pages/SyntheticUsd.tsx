    ...
    <div className="flex flex-col text-foreground">
      ...
      {loading && !data ? (
        ...
        <>
          ...
          {isViewer ? (
            <ViewerDetail selected={selected} mutations={mutations} />
          ) : (
...
// View-only account: the row list above plus the mutation log of the selected
// row. Mint, arm, scale, close and the sizing basis stay with the admin.
function ViewerDetail({
  selected,
  mutations,
}: {
  selected: SyntheticUsdPosition | null;
  mutations: SyntheticUsdMutation[];
}) {
  if (!selected) {
    return (
      <div className="px-6 py-8 text-center text-xs text-muted-foreground">
        No synthetic USD rows yet.
      </div>
    );
  }
  return <MutationHistory mutations={mutations} />;
}
