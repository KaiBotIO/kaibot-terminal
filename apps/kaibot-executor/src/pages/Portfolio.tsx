...
export default function Portfolio() {
  ...
  const exchangeBreakdown = useMemo(() => {
    const rows = exchangeSessions
      ...
      .map((s) => {
        const eq = (balances.get(sessionKey(s)) || []).reduce(
          (sum, b) => sum + (b.equity || 0),
          0,
        );
        const exposure = positions
          .filter((p) => p.exchange === s.exchangeName && (p.accountKey ?? null) === (s.accountKey ?? null))
          .reduce((sum, p) => sum + notionalOf(p), 0);
        return { exchange: connectionLabel(s), equity: eq, exposure };
      });
