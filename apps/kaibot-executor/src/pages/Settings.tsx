...
import { refreshApiConnection, setApiConnectionConfigured } from "../lib/api-connection-store";
import {
  opsApi,
  companionApi,
...
export default function Settings() {
  ...
  return isViewer ? <ViewerSettings /> : <AdminSettings />;
}

// A viewer owns nothing on the server: only the display preferences that live
// in this browser.
function ViewerSettings() {
  return (
    <div className="flex flex-col text-foreground">
      <PageHeader title="Settings" description="Display preferences for this browser." />
      <Section label="Display" flush noBorder>
        <DisplayRows />
      </Section>
    </div>
  );
...
function AdminSettings() {
  ...
  // The API key as last loaded or saved, so Save can tell a fresh pairing from
  // an unrelated settings change and only auto-connect for the former.
  const pairedKeyRef = useRef<string>('');
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.settings) {
          ...
          pairedKeyRef.current = data.settings.apiConfig?.apiKey ?? '';
        }
      }
    } catch (error) {
  ...
  const saveSettings = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (response.ok) {
        // Every pairing surface reads the shared store, so flip it here instead
        // of waiting for the next poll or a remount of this page.
        setApiConnectionConfigured(settingsToSave.apiConfig?.apiKey, settingsToSave.apiConfig?.apiUrl);
  ...
  // Assumes the current form is already persisted — /api/ws/connect reads the
  // saved backend settings, not this form.
  const connectSignalService = async () => {
    try {
  ...
  const connectWebSocket = async () => {
    ...
    // A pasted-but-unsaved key would otherwise 400 with "API configuration
    // incomplete".
    if (!(await saveSettings())) return;
    await connectSignalService();
  };

  // Saving a newly entered API key is the pairing gesture, so pair right away
  // rather than leaving Connect as a second step the user has to discover.
  const handleSave = async () => {
    const key = settings.apiConfig?.apiKey ?? '';
    const isNewKey = key.trim().length > 0 && key !== pairedKeyRef.current;
    if (!(await saveSettings())) return;
    pairedKeyRef.current = key;
    if (isNewKey && !wsStatus.connected) await connectSignalService();
  };
  ...
  return (
    <div className="flex flex-col text-foreground">
      <PageHeader
        ...
        actions={
          <Button size="sm" className="h-7 text-[11px]" onClick={handleSave} disabled={isSaving}>
      ...
      <Tabs defaultValue="api" className="flex flex-col">
        <div className="border-b border-border px-6">
          <TabsList variant="line">
            ...
            {!isDesktop() && <TabsTrigger value="users">Users</TabsTrigger>}
          </TabsList>
        </div>

        <TabsContent value="api">
          <ApiConfigTab
            apiKeyValue={settings.apiConfig?.apiKey || ''}
...
function ApiConfigTab({
  ...
  // Obfuscation must not lock the field: a stored key shows as dots until the
  // user clicks it, and an empty one is editable straight away — otherwise a
  // fresh install can never be paired with obfuscation on.
  const [editingKey, setEditingKey] = useState(false);
  const maskKey = obfuscateSensitive && !editingKey && apiKeyValue.length > 0;
        ...
        <SettingRow
          ...
          control={
            <Input
              ...
              value={maskKey ? obfuscateApiKey(apiKeyValue) : apiKeyValue}
    ...
    >
      ...
      {(
        [
          ...
          ['roll_required', 'Contract roll needed'],
...
// Browser-local preferences (timezone, brand): shared by the admin General
// tab and the viewer page.
