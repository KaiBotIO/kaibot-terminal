        ...
        <SettingRow
          ...
          control={
            <div className="flex items-center gap-2">
              ...
              {hasApiKey && (
                !wsStatus.connected ? (
                  <Button variant="outline" size="sm" onClick={onConnectWs} className="h-8 text-[11px]">
...
function AccountSizingTab() {
  ...
  const [syntheticAccounts, setSyntheticAccounts] = useState<Set<string>>(new Set());
    ...
    syntheticUsdApi
      ...
      .then((l) => {
        setSyntheticAccounts(
          new Set(
            l.positions
              .filter((p) => p.status === 'open' && p.is_factor_basis === 1)
              .map((p) => `${p.exchange}:${p.account_id}`),
          ),
        );
