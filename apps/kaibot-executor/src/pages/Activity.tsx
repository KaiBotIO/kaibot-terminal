...
import { Button, Badge, DataFreshness, DataMatrix, EmptyState, PageHeader, Section, StatStrip, Tabs, TabsContent, TabsList, TabsTrigger, downloadCsv, fmtDateTime, toCsv } from "@kaibot/shared";
import {
  Bell,
...
function typeForEvent(type: NotificationEventType): ActivityItem["type"] {
  if (type === "signal_received") return "signal";
  if (type === "order_filled" || type === "order_rejected") return "order";
  if (type === "connection_lost" || type === "connection_restored") return "connection";
  return "system";
}
...
function signalToItem(s: SignalRow): ActivityItem {
  ...
  return {
    id: `signal-${s.id}`,
    type: "signal",
    category: rejected ? "error" : executed ? "success" : "info",
    title: `${s.action?.toUpperCase?.() ?? "SIGNAL"} ${s.symbol}`,
    description: rejected
      ? `Rejected: ${s.error_message || "no reason given"}`
      : executed
        ? `Executed${s.strategy_name ? `: ${s.strategy_name}` : ""}`
        : `${s.status}${s.strategy_name ? `: ${s.strategy_name}` : ""}`,
    timestamp: s.received_at ?? "",
    symbol: s.symbol,
    error: rejected ? s.error_message ?? undefined : undefined,
    signalId: s.id,
  };
}
...
export default function Activity() {
  ...
  const openSignal = (signalId: string) => navigate(`/activity/${encodeURIComponent(signalId)}`);
          ...
          <DataMatrix
            ...
            columns={[
              ...
              },
              {
                ...
                align: "right",
                sortable: true,
                sortAccessor: (r) => (r.qtyClosed > 0 ? r.realizedNet : null),
                cell: (r) => (
                  <span className={`font-mono ${pnlClass(r.realizedNet)}`}>
                    {r.qtyClosed > 0 ? fmt(r.realizedNet) : "—"}
                  </span>
                ),
              },
              {
