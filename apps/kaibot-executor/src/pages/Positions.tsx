...
import { manualTradeApi, positionManageApi, type ManagedTrailView } from "@/lib/manual-trade-api";
import {
  positionGroupsApi,
...
function notionalOf(p: Position): number {
  return Math.abs((p.size || 0) * (p.markPrice || p.entryPrice || 0));
}
                      ...
                      >
                        ...
                        <div className="ml-auto flex items-center gap-4 font-mono text-[11px] tabular-nums">
                          ...
                          <span className="text-muted-foreground">
                            ${agg.exposure.toFixed(2)} exposure
                          </span>
                          {agg.stopRisk == null ? (
      ...
      {groupClose?.group && (
        <ConfirmDialog
          ...
          summary={[
            ...
            { label: "Exposure", value: `$${groupClose.aggregates.exposure.toFixed(2)}` },
            { label: "Order type", value: "market" },
          ]}
