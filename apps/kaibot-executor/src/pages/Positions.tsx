      ...
      },
      {
        ...
        cell: (p) => {
          ...
          if (!t || t.effectiveStop == null) {
            const f = floorFor(p);
            if (f?.manualStop != null) {
              return (
                <span
                  className="inline-flex items-center gap-1 font-mono text-[var(--kb-amber)]"
                  title={`Stop floor ${f.manualStop.toFixed(2)}${
                    f.trailingLock ? ", locked: the floor is the stop" : ", the bot trails above it"
                  }`}
                >
                  {f.trailingLock ? <Lock className="size-3" /> : <Shield className="size-3" />}
                  floor {f.manualStop.toFixed(2)}
                </span>
              );
            }
