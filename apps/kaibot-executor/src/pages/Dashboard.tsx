...
function ExchangeConnectionsStrip({
  ...
  exchangeSessions: Array<{ exchangeName: string; status: string }>;
  onManage: () => void;
}) {
  return (
    <Section
    ...
    >
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {exchangeSessions.map((session) => (
          <div key={session.exchangeName} className="flex items-center gap-1.5">
            <span className="text-[11px] capitalize text-foreground">{session.exchangeName}</span>
        ...
        <DataMatrix
          ...
          columns={[
            ...
            },
            {
              ...
              sortable: true,
              sortAccessor: (p) =>
                Math.abs((p.size || 0) * (p.markPrice || p.entryPrice || 0)),
