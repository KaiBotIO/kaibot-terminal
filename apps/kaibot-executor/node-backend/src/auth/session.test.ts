...
class FakeAuthDb {
  ...
  // Role by convention of the tests: user 1 is the admin, 2 a viewer, 3 a row
  // with a malformed role.
  getValidAuthSession(tokenHash: string) {
    const s = this.sessions.get(tokenHash)
    if (!s) return undefined
    if (s.expiresAt <= Date.now()) return undefined
    const role = s.userId === 1 ? 'admin' : s.userId === 2 ? 'viewer' : 'root'
    return { id: 1, user_id: s.userId, expires_at: new Date(s.expiresAt).toISOString(), role, username: `u${s.userId}` }
  }
...
describe('createAuthMiddleware', () => {
  ...
  const mountProtected = (opts: { trustLocal?: boolean } = {}) => {
    ...
    app.get('/protected', (c) => c.json({ ok: true, userId: c.get('userId'), role: c.get('role') }))
  }
  ...
  it('allows a valid token and exposes the userId', async () => {
    ...
    expect(await res.json()).toEqual({ ok: true, userId: 42, role: 'viewer' })
  })

  it('exposes the role of the session owner', async () => {
    mountProtected()
    const admin = await app.request('/protected', { headers: { Authorization: `Bearer ${issueSession(db as any, 1)}` } })
    expect(await admin.json()).toEqual({ ok: true, userId: 1, role: 'admin' })
    const viewer = await app.request('/protected', { headers: { Authorization: `Bearer ${issueSession(db as any, 2)}` } })
    expect(await viewer.json()).toEqual({ ok: true, userId: 2, role: 'viewer' })
  })

  it('treats a malformed role as viewer, never admin', async () => {
    mountProtected()
    const res = await app.request('/protected', { headers: { Authorization: `Bearer ${issueSession(db as any, 3)}` } })
  ...
  it('bypasses validation when trustLocal is set (desktop)', async () => {
    ...
    expect(res.status).toBe(200)
    expect(await res.json()).toEqual({ ok: true, userId: undefined, role: 'admin' })
  })
})
