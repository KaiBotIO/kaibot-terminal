...
// The role lives in auth/roles.ts (same ContextVariableMap).
declare module 'hono' {
...
// Only the two DB-constrained values exist; anything else is treated as the
// least privileged role so a malformed row can never widen access.
...
export function createAuthMiddleware(db: KaiBotDatabase, options: AuthMiddlewareOptions = {}) {
  return async (c: Context, next: Next) => {
    ...
    c.set('role', normalizeRole((session as { role?: string }).role))
    return next()
  }
}
