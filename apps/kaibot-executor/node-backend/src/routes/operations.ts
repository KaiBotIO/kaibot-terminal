...
import { backfillCommissions } from '../services/backfill-commissions.js'
import {
  assemblePortfolio,
...
export function createOperationsRoutes(
  ...
  // ── Commission backfill: ask each venue what it charged on every fill that
  //    names an order and write the fee columns. Dry run by default; only the
  //    fee columns move, and only when the value differs, so reruns report
  //    'unchanged'. Body: { dryRun?: boolean, onlyZero?: boolean, limit?: number }. ──
  app.post('/fills/backfill-commissions', async (c) => {
    let body: { dryRun?: unknown; onlyZero?: unknown; limit?: unknown } = {}
    try {
      body = (await c.req.json()) ?? {}
    } catch {
      /* no body = dry run */
    }
    const queryDry = c.req.query('dry')
    const dryRun = queryDry != null ? queryDry !== '0' : body.dryRun !== false
    const onlyZero = body.onlyZero === true || c.req.query('onlyZero') === '1'
    const rawLimit = Number(body.limit ?? c.req.query('limit'))
    const limit = Number.isFinite(rawLimit) && rawLimit > 0 ? Math.trunc(rawLimit) : undefined
    try {
      const report = await backfillCommissions(
        {
          db,
          getAdapter: async (exchange, accountId) => {
            const session = await exchangeManager.getSession('default', exchange, accountKeyOf(accountId))
            return session && session.status === 'connected' ? session.adapter : null
          },
        },
        { dryRun, onlyZero, limit },
      )
      if (!dryRun && report.updated > 0) onMutation?.()
      return c.json(report)
    } catch (error) {
      db.log('error', 'trading', 'Commission backfill failed', { error: errMsg(error) })
      return c.json({ error: 'Commission backfill failed', detail: errMsg(error) }, 500)
    }
  })
