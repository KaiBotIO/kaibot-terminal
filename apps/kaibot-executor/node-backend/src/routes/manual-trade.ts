...
import { createAdoptPositionService } from '../services/adopt-position.js'
...
export function createManualTradeRoutes(
  ...
  const adoptService = createAdoptPositionService(db, exchangeManager, deps)
  ...
  // Adopt an open manual position into the lineage of a refused/missed bot
  // entry (services/adopt-position). Candidates first, then the adoption.
  app.get('/adopt/candidates', async (c) => {
    try {
      ...
      return c.json({
        candidates: await adoptService.candidates({
          exchange,
          symbol,
  ...
  app.post('/adopt', async (c) => {
    try {
      ...
      if (typeof body?.exchange !== 'string' || !body.exchange) throw new Error('exchange required')
      if (typeof body?.symbol !== 'string' || !body.symbol) throw new Error('symbol required')
      if (typeof body?.signalId !== 'string' || !body.signalId) throw new Error('signalId required')
      if (body.stopPrice !== undefined && !(typeof body.stopPrice === 'number' && Number.isFinite(body.stopPrice) && body.stopPrice > 0)) {
        throw new Error('stopPrice must be a positive number')
      }
      return c.json(
        await adoptService.adopt({
  ...
  // can never place an entry. update/lock/unlock/remove also land on a BOT
  // position without a take-over: there they set the user's stop floor
  // (server_exit_state) and the bot keeps managing the exit above it.
  ...
  // Active edge trails (with the composed effective stop) and the stop floors
  // of bot-managed positions for the UI.
  app.get('/manage', (c) => {
    try {
      return c.json({ trails: manageService.list(), floors: manageService.listFloors() })
    } catch (error) {
