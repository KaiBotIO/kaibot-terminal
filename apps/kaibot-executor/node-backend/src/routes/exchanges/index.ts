...
import { activeRideFor } from '../../services/ride-state.js'
...
export function createExchangeRoutes(
  ...
  function rideInfoFor(exchangeName: string, p: Position) {
    try {
      const r = activeRideFor(db, exchangeName, p.accountId, p.symbol)
  ...
  function enrichPosition(session: ExchangeSession, p: Position) {
    ...
    return {
      ...
      // Ride hand-over: the server-managed ride this position is on, if any.
      ride: rideInfoFor(session.exchangeName, p),
      multiplier,
