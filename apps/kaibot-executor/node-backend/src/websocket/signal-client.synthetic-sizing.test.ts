...
describe('synthetic mode sizing', () => {
  ...
  // Kai (04/09): the armed synthetic is a sizing basis BEFORE the mint.
  it('sizes against the planned floor of an ARMED basis on an inverse venue', async () => {
    const adapter = new FakeAdapter()
    const armed = flagged({ status: 'armed', target_usd: 0, arm_trigger_price: 57_725.5, arm_holdings_coin: 2, arm_planned_usd: 115_451 })
    const { db, client } = build(sub({ factor: 2 }), armed, adapter)
    await (client as any).handleSignalInner(signal({ quantity: 5 }))
    // 5 × 2 = 10% of 115 451 → 11 545.1 USD → step 10 → 11 540
    expect(adapter.placed).toHaveLength(1)
    expect(adapter.placed[0].quantity).toBe(11_540)
    expect(db.clip('synthetic_sizing_applied')).toMatchObject({ originalQuantity: 10, adjustedQuantity: 11_540 })
    expect(db.logs.find((l) => l.message.includes('armed-cycle basis'))?.metadata.basisKind).toBe('armed')
  })

  it('sizes against the planned floor of an ARMED basis on a linear venue (needs a price)', async () => {
    const adapter = new FakeAdapter('bybit')
    const armed = flagged({ exchange: 'bybit', account_id: 'default', symbol: 'BTCUSDT', status: 'armed', target_usd: 0, arm_trigger_price: 50_000, arm_holdings_coin: 1, arm_planned_usd: 50_000 })
    const { client } = build(sub({ exchange: 'bybit', account_id: 'default' }), armed, adapter)
    await (client as any).handleSignalInner(signal({ symbol: 'BTCUSDT', quantity: 10, price: 100_000 }))
    // 10% of 50 000 = 5 000 USD at 100k → 0.05 BTC
    expect(adapter.placed[0].quantity).toBeCloseTo(0.05, 9)
  })

  it('sizes against the realized floor (holdings × fill) once the armed row is minted', async () => {
    const adapter = new FakeAdapter()
    const minted = flagged({ status: 'open', target_usd: 180_000, arm_trigger_price: 90_000, arm_holdings_coin: 2, arm_fired_trigger_price: 90_000, arm_fired_price: 89_000 })
    const { client } = build(sub(), minted, adapter)
    await (client as any).handleSignalInner(signal({ quantity: 1 }))
    expect(adapter.placed[0].quantity).toBe(1_780) // 1% of 178 000
  })
