...
describe('execution state — failed open blocks close', () => {
  it('does not close or retire an entry whose open failed', async () => {
    ...
    // 2026-09-01 (B&C zombie close): this bot's open FAILED, so the live
    // position on the same (account, symbol) belongs to another lineage — the
    // identity-scoped close must no-op (executed ack), never flatten it. The
    // pre-incident behavior placed the order here ("exchange = truth").
