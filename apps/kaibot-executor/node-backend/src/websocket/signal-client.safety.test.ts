...
class FakeDb {
  ...
  upsertDeferredEntry(row: any) {
    if (!this.deferred.has(row.signalId)) {
      this.deferred.set(row.signalId, { signal_id: row.signalId, status: 'waiting', deadline_at: row.deadlineAt })
    }
  }
  ...
  touchDeferredEntry() {}
  resolveDeferredEntry(id: string, status: string) {
...
describe('open-path market guard', () => {
  it('defers an open when a non-24/7 venue reports a stale market', async () => {
    const adapter = new FakeAdapter('tradestation')
    adapter.alwaysOpen = false
    // A weekday clock: inside the weekend gap the entry is dropped instead.
    const NOW = Date.UTC(2026, 8, 17, 21, 3)
    ;(adapter as any).getMarketStatus = async () =>
      new Map([['BTC-PERPETUAL', { symbol: 'BTC-PERPETUAL', last: 1, tradeTimeMs: NOW - 60 * 60_000 }]])
    const { db, client } = build(adapter)
    client.setClock(() => NOW)
    ...
    // Parked, not rejected: the entry waits for the venue to trade again.
    expect(db.lastStatus().status).toBe('deferred')
    expect(db.lastStatus().error).toContain('market closed')
    expect(db.deferred.get('sig-1')?.status).toBe('waiting')
