...
describe('breathing-room refinements', () => {
  ...
  it('uses server-supplied leverage from signal.metadata', async () => {
    ...
    // lev 1: 1×50000 = 50000 > 10000 → reject. metadata.leverage 10: 5000 < 10000 → allow.
    ...
    await (c2 as any).handleSignalInner(
      signal({ quantity: 1, price: 50_000, metadata: { exchange: 'tradestation', leverage: 10 } }),
    )
  ...
  it('force bypass: signal.metadata.force skips the guard (server-driven)', async () => {
    ...
    await (client as any).handleSignalInner(signal({ metadata: { exchange: 'tradestation', force: true } }))
    expect(adapter.placed).toHaveLength(1)
...
describe('breathing-room offset (opposing same-root open)', () => {
  ...
  it('only the excess past the offset is charged (partial flip)', async () => {
    // Short 1, buy 3 → 1 offsets, 2 net-new. Room for the 2 net-new (2×5000=10000
    // ≤ 12000 free, floor 0) but not all 3 (15000 > 12000) → allowed via offset.
    const adapter = new FakeAdapter(
      [bal({ equity: 12_000, initialMargin: 0, maintenanceMargin: 0 })],
      false,
      'tradestation',
      [shortMes()],
    )
