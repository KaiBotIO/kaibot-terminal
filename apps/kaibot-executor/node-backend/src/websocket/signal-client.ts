...
import { attributeVenueExit, splitFee } from '../services/exit-attribution.js';

// Fee fields a venue reported on an order result or status, in the shape the
// fill ledger stores them. Absent = booked at zero (paper, or a venue that does
// not report one); the commission backfill fills those in later.
type VenueFee = { commission?: number; feeNative?: number | null; feeCurrency?: string | null };
function feeOf(src: VenueFee | undefined | null): { commission: number; feeNative: number | null; feeCurrency: string | null } {
  return {
    commission: src?.commission ?? 0,
    feeNative: src?.feeNative ?? null,
    feeCurrency: src?.feeCurrency ?? null,
  };
}
...
// Set by the fan-out layer for a run on one of several subscriptions.
interface FanoutRunContext {
  fanout?: boolean;
  // The broker account this run's lineage lives on (close scoping).
  lineageAccount?: string;
  // server_exit_state row this update run targets (family member).
  positionId?: string;
  // Lineage this cancel run targets (family member).
...
export class SignalWebSocketClient {
  ...
  // Fan-out (services/fanout): while a wire signal runs per subscription, each
  // lineage's ack is captured here (keyed by lineage id) instead of posted, so
  // the wire signal ends in exactly one ack.
  private ackSinks = new Map<string, CapturedAck[]>();
  ...
  private async handleSignalInner(
    signal: Signal,
    opts: { resumedDeferral?: boolean; onlySubscriptionId?: string } = {},
  ) {
    ...
    try {
      ...
      // ─── Resolve subscription(s) (empty for unsubscribed) ───
      // Every active subscription of the bot, creation order: a bot routed to
      // several connections fans out below; one subscription is the plain
      // single path, unchanged.
      const signalBotId =
        (signal.metadata?.signalBotId as string | undefined) ??
        (signal.metadata?.signal_bot_id as string | undefined);
      let subs = this.activeSubscriptionsFor(signalBotId, signal, opts.onlySubscriptionId);
      let sub: any | null = subs[0] ?? null;

      // A deferred entry resumes for the subscription that parked it; one
      // that was cancelled or paused meanwhile has nothing to run on.
      if ((opts.onlySubscriptionId || fanoutSubscriptionIdOf(signal)) && subs.length === 0) {
        const reason = 'subscription no longer active';
        this.db.log('warn', 'signal', 'Resumed entry has no active subscription left', {
          signalId: signal.id,
          subscriptionId: opts.onlySubscriptionId ?? fanoutSubscriptionIdOf(signal),
        });
      ...
      if (subs.length > 1 && signalBotId) {
        return this.fanOutSignal(signal, wireSignal, subs, signalBotId);
      }
      return this.processSignalForSub(signal, wireSignal, sub, signalBotId);
    } catch (error: any) {
  ...
  // Active subscriptions of a bot in creation order. Pinned to one
  // subscription for a resumed deferral or a fan-out child signal. Defensive
  // on the listing (older test doubles only know getSubscriptionForBot).
  private activeSubscriptionsFor(
    signalBotId: string | undefined,
    signal: Signal,
    onlySubscriptionId?: string,
  ): any[] {
    if (!signalBotId) return [];
    const pinned = onlySubscriptionId ?? fanoutSubscriptionIdOf(signal);
    const lister = (this.db as { listActiveSubscriptionsForBot?: (id: string) => any[] })
      .listActiveSubscriptionsForBot;
    let subs: any[];
    if (typeof lister === 'function') {
      subs = lister.call(this.db, signalBotId) ?? [];
    } else {
      const one = this.db.getSubscriptionForBot(signalBotId);
      subs = one ? [one] : [];
    }
    return pinned ? subs.filter((s) => s.id === pinned) : subs;
  }

  // ─── Fan-out: one bot on several subscriptions (services/fanout) ───
  // Entries run once per subscription, each on its own connection and as its
  // own lineage; closes, updates and cancels run once per account that holds
  // a lineage. One failing subscription never blocks the others. The wire
  // signal ends in ONE ack, with every account's outcome in its body.
  private async fanOutSignal(
    signal: Signal,
    wireSignal: Signal,
    subs: any[],
    signalBotId: string,
    ...
    const virtualClose = signal.metadata?.virtualClose === true;
    if ((action === 'buy' || action === 'sell') && !virtualClose) {
      return this.fanOutEntry(wireSignal, subs, signalBotId);
    }
    if (action === 'close' || virtualClose) {
      return this.fanOutClose(signal, wireSignal, subs, signalBotId);
    }
    if (action === 'update') return this.fanOutUpdate(signal, wireSignal, subs, signalBotId);
    if (action === 'cancel') return this.fanOutCancel(signal, wireSignal, subs, signalBotId);
    return this.processSignalForSub(signal, wireSignal, subs[0], signalBotId, { fanout: true });
  }

  private async fanOutEntry(wireSignal: Signal, subs: any[], signalBotId: string): Promise<void> {
    const outcomes: AccountOutcome[] = [];
    for (let i = 0; i < subs.length; i++) {
      const sub = subs[i];
      const child = childSignalFor(wireSignal, sub, i);
      if (i > 0) this.recordChildSignal(child);
      const run = await this.runCollected(child, sub, signalBotId, { fanout: true });
      outcomes.push(
        outcomeFor({
          sub,
          signalId: child.id,
          ...
          threw: run.threw,
          localStatus: this.db.getSignalStatus?.(child.id),
        }),
      );
    }
    await this.finishFanout(wireSignal, outcomes);
  }

  // One close per account that holds an open lineage of this bot on the
  // symbol. No holder → the first subscription's run ends as today's no-op
  // ack. One holder → exactly the 0.4.15 routing, scoped to that account.
  private async fanOutClose(
    signal: Signal,
    wireSignal: Signal,
    subs: any[],
    signalBotId: string,
  ): Promise<void> {
    const holders = this.lineageHolders(subs, signalBotId, signal.symbol);
    const targets = subs.filter((s) => holders.some((h) => h.sub.id === s.id));
    if (targets.length === 0) {
      return this.processSignalForSub(signal, wireSignal, subs[0], signalBotId, { fanout: true });
    }
    const accountOf = (sub: any) => holders.find((h) => h.sub.id === sub.id)!.accountId;
    if (targets.length === 1) {
      return this.processSignalForSub(signal, wireSignal, targets[0], signalBotId, {
        fanout: true,
        lineageAccount: accountOf(targets[0]),
      });
    }
    const outcomes: AccountOutcome[] = [];
    for (let i = 0; i < targets.length; i++) {
      ...
      const child = childSignalFor(wireSignal, sub, i);
      if (i > 0) this.recordChildSignal(child);
      const run = await this.runCollected(child, sub, signalBotId, {
        fanout: true,
        lineageAccount: accountOf(sub),
      });
      outcomes.push(
        outcomeFor({
          sub,
  ...
  // Stop update per account: every active exit-state row of the family
  // (wire position id + derived ids), each on the account holding it.
  private async fanOutUpdate(
    signal: Signal,
    wireSignal: Signal,
    subs: any[],
    signalBotId: string,
    ...
    const wirePositionId = typeof meta.positionId === 'string' ? meta.positionId : undefined;
    const lister = (this.db as { listServerExitStatesForFamily?: (id: string) => any[] })
      .listServerExitStatesForFamily;
    const family =
      wirePositionId && typeof lister === 'function'
        ? lister.call(this.db, wirePositionId).filter((st: any) => st.active)
        : [];
    if (family.length <= 1) {
      // One lineage (or none): its own account's connection, its own row.
      const only = family[0];
      return this.processSignalForSub(
        signal,
        wireSignal,
        only ? this.subForLineage(subs, only.entry_signal_id) : subs[0],
        signalBotId,
        { fanout: true, positionId: only?.position_id },
      );
    }
    const outcomes: AccountOutcome[] = [];
    for (const state of family) {
      const owner = this.subForLineage(subs, state.entry_signal_id);
      const run = await this.runCollected(structuredClone(wireSignal), owner, signalBotId, {
        fanout: true,
      ...
      outcomes.push(
        outcomeFor({
          sub: owner,
          signalId: state.entry_signal_id,
          captured: run.captured,
          threw: run.threw,
          localStatus: this.db.getSignalStatus?.(signal.id),
        }),
      );
    }
    await this.finishFanout(wireSignal, outcomes);
  ...
  // Cancel per lineage of the entry: the wire entry plus its fan-out children.
  private async fanOutCancel(
    signal: Signal,
    wireSignal: Signal,
    subs: any[],
    signalBotId: string,
    ...
    const meta = (signal.metadata ?? {}) as Record<string, unknown>;
    const entryId = typeof meta.entrySignalId === 'string' ? meta.entrySignalId : undefined;
    const lister = (this.db as { listFanoutChildSignalIds?: (id: string) => string[] })
      .listFanoutChildSignalIds;
    const family = entryId
      ? [entryId, ...(typeof lister === 'function' ? lister.call(this.db, entryId) : [])]
      : [];
    if (family.length <= 1) {
      // One lineage: the connection that holds it, not the bot's first sub.
      return this.processSignalForSub(
        signal,
        wireSignal,
        entryId ? this.subForLineage(subs, entryId) : subs[0],
        signalBotId,
        { fanout: true },
      );
    }
    const outcomes: AccountOutcome[] = [];
    for (const lineageId of family) {
      const owner = this.subForLineage(subs, lineageId);
      const run = await this.runCollected(structuredClone(wireSignal), owner, signalBotId, {
        fanout: true,
        entrySignalId: lineageId,
      });
      outcomes.push(
        outcomeFor({
          sub: owner,
  ...
  // Accounts with an open / closing execution of this bot on the canonical
  // symbol, each with the subscription that owns the account. Creation order
  // of the subscriptions.
  private lineageHolders(
    subs: any[],
    signalBotId: string,
    canonicalSymbol: string,
  ): Array<{ accountId: string; sub: any }> {
    ...
    for (const entry of this.db.getOpenEntrySignals(canonicalSymbol, signalBotId)) {
      ...
      if (!exec || (exec.status !== 'open' && exec.status !== 'closing') || !exec.account_id) continue;
      if (byAccount.has(exec.account_id)) continue;
      const owner = subscriptionForAccount(subs, exec.account_id, accountKeyOf);
      if (owner) byAccount.set(exec.account_id, owner);
    }
    const holders: Array<{ accountId: string; sub: any }> = [];
    for (const sub of subs) {
      for (const [accountId, owner] of byAccount) {
        if (owner.id === sub.id && !holders.some((h) => h.accountId === accountId)) {
          holders.push({ accountId, sub });
        }
      }
    }
    return holders;
  ...
  // The subscription whose account holds a lineage; the first one when the
  // execution carries no account (legacy rows).
  private subForLineage(subs: any[], lineageSignalId: string): any {
    const exec = this.db.getSignalExecution(lineageSignalId);
    return (exec?.account_id && subscriptionForAccount(subs, exec.account_id, accountKeyOf)) || subs[0];
  }

  private recordChildSignal(child: Signal): void {
    try {
      this.db.recordSignal({
        id: child.id,
        strategyId: child.strategy_id,
        strategyName: child.strategy_name,
        symbol: child.symbol,
        action: child.action,
        ...
        stopLoss: child.stop_loss,
        takeProfit: child.take_profit,
        metadata: child.metadata,
      });
    } catch {
      /* redelivery: the lineage row exists */
    }
  }

  // One subscription's run with its ack captured instead of posted. A throw
  // ends that lineage as rejected and never reaches the other subscriptions.
  private async runCollected(
    child: Signal,
    sub: any,
    signalBotId: string,
    ctx: FanoutRunContext,
  ): Promise<{ captured: CapturedAck[]; threw: string | null }> {
    const captured: CapturedAck[] = [];
    this.ackSinks.set(child.id, captured);
    let threw: string | null = null;
    try {
      await this.processSignalForSub(child, structuredClone(child), sub, signalBotId, ctx);
    } catch (err: any) {
      threw = err?.message ?? String(err);
      this.db.log('error', 'signal', 'Fan-out run failed on a subscription', {
        signalId: child.id,
        subscriptionId: sub?.id,
        error: threw,
      });
      try {
        this.db.updateSignalStatus(child.id, 'rejected', undefined, threw ?? undefined);
      } catch {
        /* the lineage row may not exist */
      }
    } finally {
      this.ackSinks.delete(child.id);
    }
    return { captured, threw };
  }

  // The wire signal's single outcome: status, stored per-account outcomes,
  // one ack carrying the primary lineage's order ids plus `accounts[]`.
  private async finishFanout(wireSignal: Signal, outcomes: AccountOutcome[]): Promise<void> {
    const agg = aggregateOutcomes(outcomes);
    const setter = (this.db as { setSignalAccountOutcomes?: (id: string, o: unknown[]) => unknown })
      .setSignalAccountOutcomes;
    if (typeof setter === 'function') {
      try {
        setter.call(this.db, wireSignal.id, outcomes);
      } catch {
        /* cosmetic: the feed line */
      }
    }
    this.db.log('info', 'signal', 'Fan-out finished', {
      signalId: wireSignal.id,
      status: agg.status,
      accounts: outcomes.map((o) => ({
        account: o.accountId ?? o.accountKey ?? o.subscriptionId,
        status: o.status,
        orderId: o.orderId,
        reason: o.reason,
      })),
    });
    if (agg.status === 'none') return;
    if (agg.status === 'pending') {
      this.db.updateSignalStatus(wireSignal.id, 'pending', undefined, agg.message);
      return;
    }
    this.db.updateSignalStatus(wireSignal.id, agg.status, undefined, agg.message);
    const p = agg.primary!;
    // Server exit update: the wire-level venue stop is the WIRE account's own
    // (the lineage without a fan-out suffix, whose server_exit_state row
    // carries the wire position id) and only when THAT account reported; a
    // child's report never stands in for it (X.39). Unreported on the wire
    // account = no venueStop on the wire (the server treats it as unknown)
    // and no child order id either; every account's own report rides in
    // accounts[].
    const isUpdate = (wireSignal.action as string) === 'update';
    const wireAccount = isUpdate ? outcomes.find((o) => !isFanoutChildId(o.signalId)) : undefined;
    const wireReported = wireAccount !== undefined && 'venueStop' in wireAccount;
    await this.ackToApi(
      wireSignal.id,
      agg.status,
      p.orderId ?? undefined,
      agg.status === 'executed' ? undefined : agg.reason ?? undefined,
      (isUpdate ? wireAccount?.stopLossOrderId : p.stopLossOrderId) ?? undefined,
      p.takeProfitOrderId ?? undefined,
      p.fillPrice != null || p.fillTime != null ? { price: p.fillPrice, time: p.fillTime } : undefined,
      { accounts: outcomes, ...(wireReported ? { venueStop: wireAccount!.venueStop ?? null } : {}) },
    );
  }

  // Everything after the bot-level gates for ONE subscription: paused check,
  // close / update / cancel dispatch, venue + session, sizing, guards, the
  // order, brackets, the ack. `wireSignal` is the pre-remap copy a deferral
  // is persisted from. `ctx` is set by the fan-out layer only.
  private async processSignalForSub(
    signal: Signal,
    wireSignal: Signal,
    sub: any | null,
    signalBotId: string | undefined,
    ...
    if (signal.action === 'close') {
      return this.executeCloseSignal(signal, sub, ctx);
    }
    ...
    if ((signal.action as string) === 'update') {
      await this.handleServerExitUpdate(signal, session, exchangeName, ctx.positionId);
      return;
    }
    ...
    if ((signal.action as string) === 'cancel') {
      await this.handleCancel(signal, session, exchangeName, ctx.entrySignalId);
      return;
    }
    ...
    if (sub) {
      ...
      if (maxPositionNative) {
        ...
        // Per account when the bot fans out: another account's lineage of
        // the same bot is not this subscription's exposure.
        const existingQty = this.cumulativeOpenQtyForCap(
          canonicalSymbol,
          signalBotId ?? sub.id,
          signal.action === 'buy' ? 'long' : 'short',
          ctx.fanout ? sizingAccount : undefined,
        );
    ...
    try {
      ...
      if (
        ...
        if (slOrderId) {
          // One state row per account: a fan-out lineage keys its row on the
          // derived position id, the family is found by prefix.
          this.db.upsertServerExitState({
            positionId: fanoutScopedId(signal.metadata.positionId, signal),
      ...
      const handoverTo = signal.metadata?.handoverTo as
        ...
        | undefined;
      if (handoverTo?.botId && fanoutSubscriptionIdOf(signal)) {
        // The server adopts one position per signal: the hand-over runs on
        // the first subscription's lineage only.
        this.db.log('info', 'trading', 'Hand-over skipped on a fan-out lineage', {
          signalId: signal.id,
          botId: handoverTo.botId,
        });
      } else if (handoverTo?.botId && typeof signal.metadata?.positionId === 'string' && this.entryFilledHook) {
        try {
  ...
  private async handleServerExitUpdate(
    ...
    exchangeName: string,
    // A fan-out family member's own state row (derived position id).
    positionIdOverride?: string,
  ): Promise<void> {
    let session: { adapter: any; status?: string } = initialSession;
    const meta = (signal.metadata ?? {}) as Record<string, unknown>;
    const positionId =
      positionIdOverride ?? (typeof meta.positionId === 'string' ? meta.positionId : undefined);
    ...
    // What rests at the venue right now: the recorded stop only while its
    // order id is known; after a cancelled-then-failed replacement the row
    // keeps the old level with sl_order_id null and NOTHING rests (X.37).
    const restingNow = (): { venueStop: number | null; slOrderId: string | null } => {
      const st = positionId && typeof this.db.getServerExitState === 'function' ? this.db.getServerExitState(positionId) : null;
      const slOrderId = st?.sl_order_id ?? null;
      return { venueStop: slOrderId && typeof st?.current_stop === 'number' ? st.current_stop : null, slOrderId };
    };
    const refuse = async (reason: string) => {
      this.db.log('warn', 'signal', 'Refused server exit update', { signalId: signal.id, positionId, reason });
      this.db.updateSignalStatus(signal.id, 'rejected', undefined, reason);
      const now = restingNow();
      await this.ackToApi(signal.id, 'rejected', undefined, reason, now.slOrderId ?? undefined, undefined, undefined, { venueStop: now.venueStop });
    };
    // Every executed ack reports the stop the venue holds NOW (venueStop, plus
    // fillPrice for older servers) and its order id, so the server activates
    // the real level, not the intent (a floor or a crossed mark can keep the
    // previous stop).
    const ackExecuted = async (message?: string) => {
      const now = restingNow();
      ...
      await this.ackToApi(
        ...
        undefined,
        undefined,
        now.slOrderId ?? undefined,
        undefined,
        { price: now.venueStop, time: Date.now() },
        { venueStop: now.venueStop },
      );
    };
    const noop = async (message: string) => ackExecuted(message);
  ...
  private async handleCancel(
    ...
    exchangeName: string,
    // A fan-out family member's own lineage (derived signal id).
    entrySignalIdOverride?: string,
  ): Promise<void> {
    const meta = (signal.metadata ?? {}) as Record<string, unknown>;
    const entrySignalId =
      entrySignalIdOverride ?? (typeof meta.entrySignalId === 'string' ? meta.entrySignalId : undefined);
    if (!entrySignalId) {
  ...
  private async executeCloseSignal(signal: Signal, sub: any | null, ctx: FanoutRunContext = {}) {
    ...
    // A bot on several accounts: this close runs on ONE account's lineage.
    // Entries of the bot on another account (its own lineage, same symbol)
    // are not this close's business. Rows without an account (pre-032) stay.
    const onLineageAccount = (entryId: string): boolean => {
      if (!ctx.lineageAccount) return true;
      const exec = this.db.getSignalExecution(entryId);
      return !exec?.account_id || exec.account_id === ctx.lineageAccount;
    };
    ...
    for (const cand of subFilterCandidates.length > 0 ? subFilterCandidates : [undefined]) {
      for (const entry of this.db.getOpenEntrySignals(canonicalSymbol, cand)) {
        if (!onLineageAccount(entry.id)) continue;
        const exec = this.db.getSignalExecution(entry.id);
    ...
    const scopeAccountId =
      ctx.lineageAccount ?? (sub?.account_id as string | undefined) ?? undefined;
    ...
    for (const cand of subFilterCandidates) {
      const rows = this.db.getOpenEntrySignals(canonicalSymbol, cand).filter((e) => onLineageAccount(e.id));
      if (rows.length > 0) {
        subFilter = cand;
  ...
  private attributeExitFills(
    ...
    // Plan first so the venue's one fee can be split pro rata over the fills.
    const portions: Array<{ id: string; exec: ReturnType<KaiBotDatabase['getSignalExecution']>; portion: number }> = [];
    ...
    for (const e of entrySignals) {
      ...
      portions.push({ id: e.id, exec, portion });
      remaining -= portion;
    }
    const fees = splitFee(feeOf(fee), portions.map((p) => p.portion));
    for (const [i, { id, exec, portion }] of portions.entries()) {
      this.db.insertSignalFill({
        signalId: id,
        kind: 'exit',
        symbol,
      ...
      if (exec) {
        ...
        this.db.updateSignalExecution(id, {
          qtyClosed: newClosed,
  ...
  private async ackToApi(
    ...
    // Fan-out: every account's outcome, on the wire signal's single ack.
    // `venueStop` (server exit updates): the stop that rests at the venue
    // AFTER the attempt, explicit null = nothing rests (cancelled old stop,
    // failed placement). The server keys its stop timeline on this, never on
    // the price it asked for.
    extra?: { accounts?: AccountOutcome[]; venueStop?: number | null },
  ) {
    // A fan-out run: captured for the wire signal's single ack.
    const sink = this.ackSinks.get(signalId);
    if (sink) {
      sink.push({
        signalId,
        status,
        orderId: tradeId ?? null,
        reason: errorMessage ?? null,
        stopLossOrderId: stopLossOrderId ?? null,
        takeProfitOrderId: takeProfitOrderId ?? null,
        fillPrice: typeof fill?.price === 'number' ? fill.price : null,
        fillTime: fill?.time ?? null,
        // The lineage's venue-stop report survives the collection (X.38).
        ...(extra && 'venueStop' in extra ? { venueStop: extra.venueStop ?? null } : {}),
      });
      return;
    }
    // A fan-out lineage acking on its own later (resumed deferral, resolved
    // settlement): the server knows one signal per wire id, so an execution
    // reports under the wire id (a no-op there when it already executed) and
    // everything else stays local.
    let serverSignalId = signalId;
    if (isFanoutChildId(signalId)) {
      if (status !== 'executed') {
        this.db.log('info', 'signal', 'Ack kept local for a fan-out lineage', {
          signalId,
          status,
          reason: errorMessage,
        });
      ...
      serverSignalId = wireIdOf(signalId);
    }
    ...
    try {
      const res = await fetch(`${this.apiUrl}/api/signals/ack`, {
        ...
        body: JSON.stringify({
          ...
          // Per-account outcomes of a fan-out (fill PRICE only, never size).
  ...
  ): Promise<{ outcome: 'filled' | 'unknown'; avgPrice?: number; filledQty?: number } & VenueFee> {
  ...
  async expireDcaRungs(exchange?: string): Promise<number> {
    ...
    for (const row of rows) {
      ...
      if (status.state === 'filled') {
        ...
        this.bookRestingRungFillDelta(row, filledQty, status.averagePrice ?? null, status);
      ...
      if (status.state === 'partially_filled') {
        ...
        const delta = this.bookRestingRungFillDelta(row, cum, status.averagePrice ?? null, status);
        if (delta > 0) {
  ...
  private bookRestingRungFillDelta(
    ...
    if (delta <= 0) return 0;
    // The status fee is the order's cumulative fee; book this delta's share.
    const share = cumFilledQty > 0 ? delta / cumFilledQty : 1;
    const whole = feeOf(fee);
    const feeShare = {
      commission: whole.commission * share,
      feeNative: whole.feeNative != null ? whole.feeNative * share : null,
      feeCurrency: whole.feeCurrency,
    };
  ...
  private async reconcileRestingRungAfterFailedCancel(
    ...
    const cum = status.filledQuantity && status.filledQuantity > 0 ? status.filledQuantity : 0;
    if (status.state === 'filled') {
      this.bookRestingRungFillDelta(row, cum > 0 ? cum : row.qty, status.averagePrice ?? null, status);
      this.db.deleteDcaRestingRung(row.order_id);
      return 'settled';
    }
    if (status.state === 'partially_filled') {
    ...
    if (status.state === 'cancelled' || status.state === 'rejected') {
      this.bookRestingRungFillDelta(row, cum, status.averagePrice ?? null, status);
      this.db.deleteDcaRestingRung(row.order_id);
      return 'dropped';
    }
    return 'kept';
  ...
  private async settleEntry(
    ...
    accountId: string,
    requestedQty: number,
  ): Promise<{ outcome: 'filled' | 'unknown' | 'rejected'; filledQty: number; avgPrice?: number; reason?: string } & VenueFee> {
    ...
    if (settled.status === 'cancelled') {
      ...
      if (settled.filledQuantity && settled.filledQuantity > 0) {
        return { outcome: 'filled', filledQty: settled.filledQuantity, avgPrice: settled.averagePrice, ...feeOf(settled) };
      }
  ...
  async reportVenueExitToApi(
    ...
    // A bot on several accounts holds one server position: it stays open on
    // the server while ANY account still holds a lineage. Only the last
    // account's exit is reported, under the wire position id.
    const wirePositionId = wireIdOf(positionId);
    const lister = (this.db as { listServerExitStatesForFamily?: (id: string) => any[] })
      .listServerExitStatesForFamily;
    if (typeof lister === 'function') {
      const stillHeld = lister
        .call(this.db, wirePositionId)
        .filter((st: any) => st.position_id !== positionId && st.active);
      if (stillHeld.length > 0) {
        this.db.log('info', 'trading', 'Venue exit kept local: other accounts still hold the lineage', {
          positionId,
          stillHeld: stillHeld.map((st: any) => st.position_id),
        });
        return;
      }
    }
  ...
  private cumulativeOpenQtyForCap(
    ...
    for (const e of this.db.getOpenEntrySignals(canonicalSymbol, subFilter)) {
      ...
      const exec = this.db.getSignalExecution(e.id);
      if (account && exec?.account_id && exec.account_id !== account) continue;
  ...
  private async resumeDeferredEntry(row: DeferredEntryRow): Promise<void> {
    ...
    await withOrderLock(this.resolveOrderLockKey(signal), () =>
      this.handleSignalInner(signal, {
        resumedDeferral: true,
        onlySubscriptionId: row.subscription_id ?? undefined,
      }),
    );
