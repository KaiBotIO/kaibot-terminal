...
export class SignalWebSocketClient {
  ...
  private async handleServerExitUpdate(
    ...
    // Every executed ack reports the stop the venue holds NOW (fillPrice) and
    // its order id, so the server activates the real level, not the intent
    // (a floor or a crossed mark can keep the previous stop).
    const ackExecuted = async (message?: string) => {
      const after = typeof this.db.getServerExitState === 'function' ? this.db.getServerExitState(positionId!) : null;
      const venueStop = after?.current_stop ?? null;
      ...
      await this.ackToApi(
        ...
        undefined,
        undefined,
        after?.sl_order_id ?? undefined,
        undefined,
        { price: typeof venueStop === 'number' ? venueStop : null, time: Date.now() },
      );
    };
