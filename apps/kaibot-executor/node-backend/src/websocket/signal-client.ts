...
export class SignalWebSocketClient {
  ...
  private async handleServerExitUpdate(
    ...
    if (!isFavourableStop(newStop, state.current_stop, state.direction)) {
    ...
    if (
      markPrice != null &&
      markPrice > 0 &&
      (state.direction === 'long' ? newStop >= markPrice : newStop <= markPrice)
    ) {
    ...
    // as the local trail loop). The bracket row is the fallback id source.
    // Never cancel an order whose bracket row sits on ANOTHER account
    // (2026-09-22: the acct1 adoption had taken over the default connection's
    // stop; the update then cancelled it through the acct1 session). The row
    // stays untouched, the new stop still goes up so this account's position
    // is protected, and the error names the stray order for the operator.
    const bracket = this.db.getSignalBracket(state.entry_signal_id);
    const pair = typeof this.db.getBracketPair === 'function' ? this.db.getBracketPair(state.entry_signal_id) : undefined;
    const oldStopId = state.sl_order_id ?? bracket?.stop_loss_order_id ?? null;
    const oldStopOnThisAccount =
      !oldStopId ||
      !pair ||
      pair.sl_order_id !== oldStopId ||
      rowOnAccount(pair.account_id, lineageAccount ?? ((live as any).accountId as string | undefined));
    if (oldStopId && !oldStopOnThisAccount) {
      this.db.log('error', 'trading', 'Exit update: old stop belongs to another account — NOT cancelled', {
        signalId: signal.id,
        positionId,
        slOrderId: oldStopId,
        bracketAccountId: pair?.account_id ?? null,
        lineageAccountId: lineageAccount,
      });
    } else if (oldStopId) {
      try {
        await session.adapter.cancelOrder(oldStopId, { symbol: state.symbol });
      } catch (err: any) {
        this.db.log('warn', 'trading', 'Exit update: cancel old stop failed', {
          signalId: signal.id, slOrderId: oldStopId, error: err.message,
        });
      }
    }
    ...
    try {
      const placed = await session.adapter.placeOrder({
        accountId: (live as any).accountId ?? 'default',
        symbol: state.symbol,
      ...
      this.db.updateSignalOrderIds(state.entry_signal_id, placed.orderId, tpId);
      // A user-configured local trail on the same position keeps working — it
      // amends the same resting stop, so hand it the fresh id and floor.
      if (typeof this.db.updateLocalTrail === 'function') {
        this.db.updateLocalTrail(state.entry_signal_id, {
          slOrderId: placed.orderId,
          currentStop: newStop,
        });
      }
      this.db.log('info', 'trading', 'Server exit update applied', {
        signalId: signal.id, positionId, exitSeq, newStop,
      });
      this.db.updateSignalStatus(signal.id, 'executed');
      await this.ackToApi(signal.id, 'executed', undefined);
    } catch (err: any) {
      // Old stop already cancelled — drop the stale id so nothing cancels a
      // dead order; seq is NOT advanced so the server's retry can re-place.
      this.db.log('error', 'trading', 'Exit update: place new stop failed — position is UNPROTECTED until retried', {
        signalId: signal.id, positionId, error: err.message,
      });
      this.db.applyServerExitUpdate(positionId, {
      ...
      this.db.updateSignalOrderIds(state.entry_signal_id, undefined, tpId);
