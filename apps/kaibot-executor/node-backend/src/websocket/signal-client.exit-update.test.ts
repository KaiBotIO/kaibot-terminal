...
class FakeDb {
  ...
  applyServerExitUpdate(positionId: string, patch: any) {
    ...
    if (patch.engineStop !== undefined) row.engine_stop = patch.engineStop
  }
  updateServerExitStopFloor(positionId: string, fields: any) {
    const row = this.exitStates.get(positionId)
    if (!row) return
    if (fields.manualStop !== undefined) row.manual_stop = fields.manualStop
    if (fields.trailingLock !== undefined) row.trailing_lock = fields.trailingLock ? 1 : 0
  }
...
// Stop floor (migration 038): the user pins a stop under the bot's; server
// updates compose with it. Favourable-only runs against the bot's OWN stop.
describe('update: with a stop floor set by the user', () => {
  async function withFloor(adapter: FakeAdapter, client: SignalWebSocketClient, db: FakeDb, floor: number, lock = false) {
    await openAuthorizedLong(adapter, client)
    // The floor was set through /manage: venue stop already at the floor.
    db.updateServerExitStopFloor(POS, { manualStop: floor, trailingLock: lock })
    const row = db.getServerExitState(POS)
    row.engine_stop = row.current_stop
    row.current_stop = floor
    adapter.cancelled = []
  }

  it('a bot stop below the floor is recorded, the venue stop stays at the floor', async () => {
    const adapter = new FakeAdapter()
    const { db, client } = build(adapter)
    await withFloor(adapter, client, db, 58000)
    const placedBefore = adapter.placed.length
    ...
    const state = db.getServerExitState(POS)
    expect(state.last_exit_seq).toBe(1)
    expect(state.engine_stop).toBe(57000)
    expect(state.current_stop).toBe(58000)
  })

  it('a bot stop above the floor moves the venue stop', async () => {
    const adapter = new FakeAdapter()
    const { db, client } = build(adapter)
    await withFloor(adapter, client, db, 58000)
    const placedBefore = adapter.placed.length

    await (client as any).handleSignalInner(updateSignal(59000, 1))

    expect(db.lastStatus().status).toBe('executed')
    expect(adapter.placed.length).toBe(placedBefore + 1)
    expect(adapter.placed[adapter.placed.length - 1].stopPrice).toBe(59000)
    const state = db.getServerExitState(POS)
    expect(state.engine_stop).toBe(59000)
    expect(state.current_stop).toBe(59000)
  })

  it('favourable-only is judged against the bot stop, not the floor', async () => {
    const adapter = new FakeAdapter()
    const { db, client } = build(adapter)
    await withFloor(adapter, client, db, 58000)
    // Bot 55000 → 56000: sharper for the bot, still under the floor.
    await (client as any).handleSignalInner(updateSignal(56000, 1))
    expect(db.lastStatus().status).toBe('executed')
    expect(db.getServerExitState(POS).engine_stop).toBe(56000)
    // Bot 56000 → 55500: a loosening, refused even though the floor holds.
    await (client as any).handleSignalInner(updateSignal(55500, 2))
    expect(db.lastStatus().status).toBe('rejected')
    expect(db.lastStatus().error).toContain('not favourable')
    expect(db.getServerExitState(POS).last_exit_seq).toBe(1)
  })

  it('locked: the bot stop is recorded but never placed', async () => {
    const adapter = new FakeAdapter()
    const { db, client } = build(adapter)
    await withFloor(adapter, client, db, 58000, true)
    const placedBefore = adapter.placed.length

    await (client as any).handleSignalInner(updateSignal(59500, 1))

    expect(db.lastStatus().status).toBe('executed')
    expect(adapter.placed.length).toBe(placedBefore)
    const state = db.getServerExitState(POS)
    expect(state.engine_stop).toBe(59500)
    expect(state.current_stop).toBe(58000)
    expect(state.last_exit_seq).toBe(1)
  })
})
