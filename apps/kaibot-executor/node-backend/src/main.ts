...
// Role gate (auth/roles.ts): admin passes, a viewer only reaches the read
// allowlist. Runs after the session check so the role comes from the DB row.
...
app.post('/api/auth/login', async (c) => {
  ...
  try {
    ...
    const user = isValid ? db.getUserByUsername(username) : undefined
    if (user) {
      const token = issueSession(db, user.id)
      db.log('info', 'system', 'User logged in', { username, role: user.role })
      return c.json({ token, username, role: user.role })
    }
...
// Who holds this session. The UI keys its view-only mode on `role`; desktop
// (trusted local caller) resolves to the admin row.
app.get('/api/auth/session', async (c) => {
  try {
    const userId = c.get('userId') as number | undefined
    const user = userId != null ? db.getUserById(userId) : db.getAdminUser()
    if (!user) return c.json({ error: 'No user found' }, 404)
    return c.json({
      username: user.username,
      role: user.role,
      groups: user.role === 'admin' ? await remoteUserGroups() : [],
    })
  } catch (error) {
    db.log('error', 'system', 'Failed to get session', { error: errMsg(error) })
...
// Studio groups of the paired (admin) account, best effort.
async function remoteUserGroups(): Promise<string[]> {
  try {
    const user = db.getAdminUser()
    ...
    if (apiUrl && apiKey) {
      ...
      if (remote.ok) {
        ...
        if (Array.isArray(remoteGroups)) return remoteGroups
      }
    }
  } catch (err) {
...
app.get('/api/auth/me', async (c) => {
  try {
    ...
    const groups = await remoteUserGroups()
...
app.get('/api/performance/analytics', (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      byAccount: buildAccountScorecards(closed),
      ...
      closedTrades: closed.map((r) => ({
        ...
        grossPnl: r.grossPnl ?? r.realizedPnl,
