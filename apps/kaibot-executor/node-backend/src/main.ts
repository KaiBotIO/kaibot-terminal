...
// the legacy KaiBotWeb session, active until the OAuth app API key is
// approved). Set TRADESTATION_USE_OAUTH=true to flip both this backend and the
// frontend (src/lib/config.ts reads the VITE_ alias of the same var).
import { resolveTradestationUseOAuth } from './services/tradestation-auth-flag.js'
const TRADESTATION_USE_OAUTH = resolveTradestationUseOAuth()
...
const syntheticRebalancer = new SyntheticRebalancer({
  db,
  exchangeManager,
  service: createSyntheticUsdService(db, exchangeManager),
  notifications: notificationBus,
})
...
// 'tradestation' adapter selected by TRADESTATION_USE_OAUTH (see top of file).
...
// its access_token. CouchDB is active until the OAuth app key arrives.
  ...
  try {
    ...
    for (const session of sessions) {
      ...
      try {
        ...
        for (const p of live) {
          if (Math.abs(p.size) > 0) positions.push({ ...p, exchange_name: session.exchangeName })
        }
      } catch (err) {
...
app.get('/api/performance/analytics', (c) => {
  ...
  try {
    const execs = db.all(
      `SELECT signal_id, symbol, direction, status, qty_opened, qty_closed FROM signal_executions WHERE status = 'closed'`,
      [],
    ) as Array<{
      signal_id: string
      symbol: string
    ...
    let closed = closedRowsFromExecutions(execs, fillsBySignal)
    if (days > 0) {
      const since = Date.now() - days * 86_400_000
      closed = closed.filter((r) => r.closedAt >= since)
    }
...
const exchangeRoutes = createExchangeRoutes(db, exchangeManager)
...
const operationsRoutes = createOperationsRoutes(db, exchangeManager, alerting, pushStateNow)
...
const manualTradeRoutes = createManualTradeRoutes(db, exchangeManager, {
  registerBracket: (exchange, signalId, slOrderId, tpOrderIds) =>
    signalClient.registerBracket(exchange, signalId, slOrderId, tpOrderIds),
  retireBracket: (exchange, signalId) => signalClient.retireBracket(exchange, signalId),
  cancelEntryRungs: (exchange, signalIds) =>
    signalClient.cancelRestingDcaRungsForSignals(exchange, signalIds),
})
...
const positionGroupRoutes = createPositionGroupRoutes(db, exchangeManager, {
  retireBracket: (exchange, signalId) => signalClient.retireBracket(exchange, signalId),
  cancelEntryRungs: (exchange, signalIds) =>
    signalClient.cancelRestingDcaRungsForSignals(exchange, signalIds),
})
...
// boot path selected by TRADESTATION_USE_OAUTH (see top of file). Deribit /
