...
export interface PlannedTrail {
  ...
  maxPoints?: number
  // Break-even fee buffer (fraction). User-authored; absent = no break-even
  // floor. The executor never substitutes a KaiBot default.
  breakevenFee?: number
}
...
export interface Signal {
  ...
  received_at: Date
  processed_at?: Date
  status: 'pending' | 'executed' | 'rejected' | 'expired' | 'closed'
  trade_id?: number
...
export type SyntheticUsdStatus = 'open' | 'closed'
...
export type SyntheticUsdMutationKind = 'mint' | 'scale_up' | 'scale_down' | 'close' | 'auto_rebalance'
