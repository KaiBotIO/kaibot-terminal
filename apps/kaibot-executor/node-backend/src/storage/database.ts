...
export class KaiBotDatabase {
  ...
  private migrate() {
    ...
    if (!appliedMigrations.has('exchange_tables')) {
      ...
      const exchangeSchema = readFileSync(join(__dirname, 'migrations', '002_exchanges.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('executor_subscriptions')) {
      ...
      const subSchema = readFileSync(join(__dirname, 'migrations', '003_subscriptions.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('signal_stops')) {
      ...
      const stopsSchema = readFileSync(join(__dirname, 'migrations', '004_signal_stops.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('auth_sessions')) {
      ...
      const authSchema = readFileSync(join(__dirname, 'migrations', '005_auth_sessions.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('drop_dead_position_tables')) {
      ...
      const dropSchema = readFileSync(join(__dirname, 'migrations', '006_drop_dead_position_tables.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('execution_state')) {
      ...
      const execSchema = readFileSync(join(__dirname, 'migrations', '007_execution_state.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('reconciliation')) {
      ...
      const reconSchema = readFileSync(join(__dirname, 'migrations', '008_reconciliation.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('account_sizes')) {
      ...
      const sizesSchema = readFileSync(join(__dirname, 'migrations', '009_account_sizes.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('synthetic_usd')) {
      ...
      const synthSchema = readFileSync(join(__dirname, 'migrations', '010_synthetic_usd.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('order_settlement_dedup')) {
      ...
      const dedupSchema = readFileSync(join(__dirname, 'migrations', '011_order_settlement_dedup.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('tp_ladder')) {
      ...
      const tpSchema = readFileSync(join(__dirname, 'migrations', '012_tp_ladder.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('local_trail_state')) {
      ...
      const trailSchema = readFileSync(join(__dirname, 'migrations', '013_local_trail_state.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('portfolio_sync')) {
      ...
      const syncSchema = readFileSync(join(__dirname, 'migrations', '014_portfolio_sync.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('margin_guards')) {
      const marginSchema = readFileSync(join(__dirname, 'migrations', '015_margin_guards.sql'), 'utf-8')
      this.db.exec(marginSchema)
      this.db.run('INSERT INTO migrations (name) VALUES (?)', ['margin_guards'])
    }

    if (!appliedMigrations.has('guardrails')) {
      const guardrailsSchema = readFileSync(join(__dirname, 'migrations', '016_guardrails.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('bot_configs')) {
      ...
      const botSchema = readFileSync(join(__dirname, 'migrations', '017_bot_configs.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('bot_config_execution_target')) {
      const execTargetSchema = readFileSync(
        join(__dirname, 'migrations', '018_bot_config_execution_target.sql'),
        'utf-8',
      )
    ...
    if (!appliedMigrations.has('synthetic_usd_leverage_cap')) {
      const capSchema = readFileSync(
        join(__dirname, 'migrations', '019_synthetic_usd_leverage_cap.sql'),
        'utf-8',
      )
    ...
    if (!appliedMigrations.has('companion')) {
      ...
      const companionSchema = readFileSync(join(__dirname, 'migrations', '020_companion.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('companion_e2e')) {
      ...
      const schema = readFileSync(join(__dirname, 'migrations', '021_companion_e2e.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('manual_positions')) {
      const schema = readFileSync(join(__dirname, 'migrations', '022_manual_positions.sql'), 'utf-8')
    ...
    if (!appliedMigrations.has('synthetic_usd_auto_rebalance')) {
      const schema = readFileSync(
        join(__dirname, 'migrations', '024_synthetic_usd_auto_rebalance.sql'),
        'utf-8',
      )
      const statements = schema.split(';').map((s) => s.trim()).filter((s) => s.length > 0)
      for (const stmt of statements) {
    ...
    if (!appliedMigrations.has('subscription_size_unit')) {
      const schema = readFileSync(
        join(__dirname, 'migrations', '025_subscription_size_unit.sql'),
        'utf-8',
      )
      const statements = schema.split(';').map((s) => s.trim()).filter((s) => s.length > 0)
      for (const stmt of statements) {
    ...
    if (!appliedMigrations.has('dca_resting_rungs')) {
      const schema = readFileSync(
        join(__dirname, 'migrations', '026_dca_resting_rungs.sql'),
        'utf-8',
      )
      const statements = schema.split(';').map((s) => s.trim()).filter((s) => s.length > 0)
      for (const stmt of statements) {
    ...
    if (!appliedMigrations.has('partial_fill_tracking')) {
      const schema = readFileSync(
        join(__dirname, 'migrations', '027_partial_fill_tracking.sql'),
        'utf-8',
      )
      const statements = schema.split(';').map((s) => s.trim()).filter((s) => s.length > 0)
      for (const stmt of statements) {
    ...
    if (!appliedMigrations.has('position_trail')) {
      const schema = readFileSync(
        join(__dirname, 'migrations', '028_position_trail.sql'),
        'utf-8',
      )
      // ALTER ADD COLUMN is not idempotent in sqlite; strip comment lines (a ';'
    ...
    if (!appliedMigrations.has('position_managers')) {
      const schema = readFileSync(
        join(__dirname, 'migrations', '029_position_managers.sql'),
        'utf-8',
      )
      this.db.exec(schema)
    ...
    if (!appliedMigrations.has('position_groups')) {
      const schema = readFileSync(
        join(__dirname, 'migrations', '030_position_groups.sql'),
        'utf-8',
      )
      this.db.exec(schema)
  ...
  upsertSubscription(sub: {
    ...
    return this.db.run(
      `INSERT INTO executor_subscriptions
        (id, signal_bot_id, bot_name, selected_markets, factor, max_position_size, max_concurrent_trades, exchange, account_id, status, size_unit, updated_at)
  ...
  setBotConfigStatus(id: string, status: 'running' | 'paused' | 'stopped') {
    return this.db.run(
  ...
  getSignalCountForSubscription(subscriptionId: string, sinceHours: number = 24): number {
    ...
    ).get(subscriptionId, sinceHours) as any
    return row?.count ?? 0
  }
    ...
    try {
      this.db.run(
        `INSERT INTO signal_executions
           (signal_id, symbol, exchange, direction, status, qty_opened, qty_closed, error_reason, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          row.signalId,
          row.symbol,
  ...
  upsertBracketPair(row: {
    ...
    return this.db.run(
      `INSERT INTO bracket_pairs (signal_id, exchange, sl_order_id, tp_order_id, tp_order_ids, created_at)
       VALUES (?, ?, ?, ?, ?, ?)
      ...
      [row.signalId, row.exchange, row.slOrderId ?? null, firstTp, tpIds ? JSON.stringify(tpIds) : null, Date.now()],
    )
  }
  ...
  // account. Used to enforce ONE stop-owner per position and by the manual
  // close path to retire the trail with the position.
  findActiveTrailsForSymbol(exchange: string, symbol: string): LocalTrailStateRow[] {
    return this.db
      .query(
  ...
  findLatestRetiredTrailForSymbol(exchange: string, symbol: string): LocalTrailStateRow | undefined {
    return this.db
      .query(
  ...
  // so a manual close can cancel their resting bracket legs (OCO cleanup).
  listManualEntrySignalIds(exchange: string, symbol: string): string[] {
    return (
      this.db
        .query(
  ...
  latestReconciliationPerSymbol(exchange?: string): ReconciliationRow[] {
    const sql = `
      ...
      JOIN (
        ...
        ON r.exchange = latest.exchange AND r.symbol = latest.symbol AND r.ts = latest.ts`
    return (exchange
  ...
  getFactorBasisSyntheticUsdPosition(): SyntheticUsdPositionRow | null {
    const row = this.db
      .query(
        `SELECT * FROM synthetic_usd_positions
         WHERE is_factor_basis = 1 AND status = 'open' LIMIT 1`,
      )
      .get() as SyntheticUsdPositionRow | undefined
    return row ?? null
  }
  ...
  listSyntheticUsdPositions(includeClosed = false): SyntheticUsdPositionRow[] {
    const sql = includeClosed
      ...
      : "SELECT * FROM synthetic_usd_positions WHERE status = 'open' ORDER BY created_at DESC"
  ...
  // Mark one position as the factor basis, clearing any other. Atomic.
  setSyntheticUsdFactorBasis(id: string, enabled: boolean) {
    this.transaction(() => {
      this.db.run('UPDATE synthetic_usd_positions SET is_factor_basis = 0, updated_at = ?', [
        Date.now(),
      ])
      if (enabled) {
        this.db.run(
    ...
    return this.db.run(
      `INSERT INTO synthetic_usd_mutations
        (position_id, kind, target_usd_before, target_usd_after,
         short_size_before, short_size_after, order_id, order_side, order_qty, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        row.position_id,
        row.kind,
