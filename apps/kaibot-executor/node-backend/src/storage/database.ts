...
export class KaiBotDatabase {
    ...
    return this.db.run(
      `INSERT INTO server_exit_state
         (position_id, entry_signal_id, exchange, symbol, direction,
          last_exit_seq, current_stop, sl_order_id, active, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, 0, ?, ?, 1, ?, ?)
      ...
      [
        ...
        row.currentStop ?? null, row.slOrderId ?? null, now, now,
      ],
    )
  }
  ...
  applyServerExitUpdate(positionId: string, patch: {
    ...
    return this.db.run(
      `UPDATE server_exit_state
       SET last_exit_seq = ?, current_stop = ?, sl_order_id = ?, updated_at = ?
       WHERE position_id = ?`,
      [patch.exitSeq, patch.currentStop ?? null, patch.slOrderId ?? null, Date.now(), positionId],
    )
  }
