...
export class KaiBotDatabase {
  ...
  private migrate() {
    ...
    if (!appliedMigrations.has('signal_account_outcomes')) {
      this.execTolerantAlter(sqlAsset('migrations/039_signal_account_outcomes.sql'))
      this.db.run('INSERT INTO migrations (name) VALUES (?)', ['signal_account_outcomes'])
    }

    if (!appliedMigrations.has('fill_fee_native')) {
      this.execTolerantAlter(sqlAsset('migrations/040_fill_fee_native.sql'))
      this.db.run('INSERT INTO migrations (name) VALUES (?)', ['fill_fee_native'])
    }

    if (!appliedMigrations.has('user_roles')) {
      this.execTolerantAlter(sqlAsset('migrations/041_user_roles.sql'))
      this.db.run('INSERT INTO migrations (name) VALUES (?)', ['user_roles'])
    }
  }
  ...
  // Admin = the operator account. Viewers never own settings, pairing or
  // sessions with the signal service, so every "the user's settings" read
  // stays pinned to this row.
  async hasAdminUser(): Promise<boolean> {
    const count = this.db.query("SELECT COUNT(*) as count FROM users WHERE role = 'admin'").get() as any
    return count.count > 0
  }

  getAdminUser() {
    return this.db.query("SELECT * FROM users WHERE role = 'admin' ORDER BY id LIMIT 1").get() as any
  }

  getUserByUsername(username: string): UserAccount | undefined {
    return (this.db
      .query('SELECT id, username, role, created_at, last_login FROM users WHERE username = ?')
      .get(username) as UserAccount | null) ?? undefined
  }

  getUserById(id: number): UserAccount | undefined {
    return (this.db
      .query('SELECT id, username, role, created_at, last_login FROM users WHERE id = ?')
      .get(id) as UserAccount | null) ?? undefined
  }

  listUsers(): UserAccount[] {
    return this.db
      .query('SELECT id, username, role, created_at, last_login FROM users ORDER BY id')
      .all() as UserAccount[]
  }

  async createViewerUser(username: string, password: string): Promise<UserAccount> {
    const bcrypt = await import('bcryptjs')
    const hashedPassword = await bcrypt.hash(password, 10)
    this.db.run(
  ...
  // A new password invalidates every session the account holds.
  async setUserPassword(userId: number, password: string) {
    const bcrypt = await import('bcryptjs')
    const hashedPassword = await bcrypt.hash(password, 10)
    this.db.run('UPDATE users SET password_hash = ? WHERE id = ?', [hashedPassword, userId])
    this.deleteAuthSessionsForUser(userId)
  }
  ...
  // Returns the session row (with the owner's role) when the token hash is
  // known and not expired.
  getValidAuthSession(tokenHash: string): AuthSessionRow | undefined {
    return (this.db.query(
      `SELECT s.id, s.user_id, s.expires_at, u.role, u.username FROM auth_sessions s
       JOIN users u ON u.id = s.user_id
       WHERE s.token_hash = ? AND s.expires_at > CURRENT_TIMESTAMP
       LIMIT 1`,
    ).get(tokenHash) as AuthSessionRow | null) ?? undefined
  }
  ...
  deleteAuthSessionsForUser(userId: number) {
    return this.db.run('DELETE FROM auth_sessions WHERE user_id = ?', [userId])
  }
  ...
  getRecentSignals(limit: number = 50) {
    return this.db.query(
      `SELECT s.*,
         ...
         -- the subscription that would have placed it: the bot's FIRST one (a
         -- bot on several accounts must not multiply the row).
         LEFT JOIN executor_subscriptions sub
                ON sub.rowid = (
                     SELECT x.rowid FROM executor_subscriptions x
                      WHERE x.signal_bot_id = COALESCE(
                              json_extract(s.metadata, '$.signalBotId'),
                              json_extract(s.metadata, '$.signal_bot_id'))
                      ORDER BY x.rowid ASC LIMIT 1)
        -- A fan-out child (one bot on several accounts) is a lineage of its
        -- wire signal, not a signal of its own: the wire row carries the
        -- per-account outcomes.
        WHERE json_extract(s.metadata, '$.fanoutOf') IS NULL
        ORDER BY s.received_at DESC
        LIMIT ?`,
    ).all(limit)
  }

  // Lineage ids a wire signal fanned out to (services/fanout), creation order.
  listFanoutChildSignalIds(wireSignalId: string): string[] {
    return (
      this.db
        .query(
          "SELECT id FROM signals WHERE json_extract(metadata, '$.fanoutOf') = ? ORDER BY rowid ASC",
        )
        .all(wireSignalId) as Array<{ id: string }>
    ).map((r) => r.id)
  }

  setSignalAccountOutcomes(wireSignalId: string, outcomes: unknown[]) {
    return this.db.run('UPDATE signals SET account_outcomes = ? WHERE id = ?', [
  ...
  private static readonly NOT_FANOUT_CHILD = "json_extract(metadata, '$.fanoutOf') IS NULL"
    ...
    const row = this.db.query(
      `SELECT COUNT(*) as count FROM signals
       WHERE ${KaiBotDatabase.SIGNAL_OWNER_MATCH}
         AND ${KaiBotDatabase.NOT_FANOUT_CHILD}
  ...
  insertSignalFill(fill: {
    ...
    qty: number
    price?: number | null
    // USD. The native pair is the venue's own figure when it charged in coin.
    commission?: number
    ...
    return this.db.run(
      `INSERT INTO signal_fills (signal_id, kind, symbol, side, qty, price, commission, fee_native, fee_currency, order_id, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        fill.signalId,
        fill.kind,
  ...
  // Commission backfill: the fee columns only, nothing else on the row.
  updateSignalFillFee(id: number, fee: { commission: number; feeNative?: number | null; feeCurrency?: string | null }) {
    return this.db.run(
      'UPDATE signal_fills SET commission = ?, fee_native = ?, fee_currency = ? WHERE id = ?',
      [fee.commission, fee.feeNative ?? null, fee.feeCurrency ?? null, id],
    )
  }

  // Fills that name a venue order, with the execution's venue and account so
  // the backfill can ask the right connection. Oldest first.
  listFillsWithOrderId(limit = 5000): FillWithVenueRow[] {
    return this.db
      .query(
        `SELECT f.id, f.signal_id, e.exchange, e.account_id, f.symbol, f.kind, f.side, f.qty, f.price,
                f.commission, f.fee_native, f.fee_currency, f.order_id, f.created_at
           FROM signal_fills f
  ...
  // Every account's exit state for one server position: the wire position id
  // (first subscription) plus the derived ids of the other subscriptions.
  listServerExitStatesForFamily(wirePositionId: string): ServerExitStateRow[] {
    return this.db
      .query(
        "SELECT * FROM server_exit_state WHERE position_id = ? OR position_id LIKE ? ORDER BY rowid ASC",
      )
      .all(wirePositionId, `${wirePositionId}~%`) as ServerExitStateRow[]
  }
