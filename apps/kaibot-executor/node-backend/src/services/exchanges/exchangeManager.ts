...
export class ExchangeManager extends EventEmitter {
  ...
  async connectExchange(
    ...
    const sessionKey = `${userId}:${exchangeName}`
    ...
    const adapter = this.sessions.get(sessionKey)?.adapter ?? this.createAdapter(exchangeName)
    if (!adapter) {
    ...
    try {
      ...
      await this.db.run(`
        INSERT OR REPLACE INTO exchange_connections (
          ...
          encrypted_credentials, is_active, last_refresh
        ) VALUES (?, ?, ?, ?, ?, 1, ?)
      `, [
        sessionKey,
        userId,
      ...
      adapter.subscribeToUpdates((data) => {
        this.emit('exchangeUpdate', { userId, exchangeName, ...data })
      })

      console.log(`Connected to ${exchangeName} for user ${userId}`)
    } catch (error: any) {
  ...
  async disconnectExchange(userId: string, exchangeName: string): Promise<void> {
    const sessionKey = `${userId}:${exchangeName}`
    ...
    console.log(`Disconnected from ${exchangeName} for user ${userId}`)
  }

  async completeOAuthFlow(userId: string, exchangeName: string, code: string): Promise<void> {
    const sessionKey = `${userId}:${exchangeName}`
    const session = this.sessions.get(sessionKey)
    ...
    if (storable) {
      ...
      `, [sessionKey, userId, exchangeName, 'oauth', encryptedCredentials, Date.now()])
    }
  ...
  async getSession(userId: string, exchangeName: string): Promise<ExchangeSession | undefined> {
    const sessionKey = `${userId}:${exchangeName}`
  ...
  private scheduleReconnect(sessionKey: string): void {
    ...
    const attempt = this.reconnectAttempts.get(sessionKey) ?? 0
    const delay = Math.min(60_000, 1_000 * 2 ** Math.min(attempt, 6)) // 1s,2s,…64s capped 60s
  ...
  private async reconnectSession(sessionKey: string): Promise<void> {
    ...
    try {
      ...
      await this.connectExchange(session.userId, session.exchangeName, credentials)
  ...
  async restoreSessions(): Promise<void> {
    ...
    for (const connection of connections) {
      ...
      try {
        await this.connectExchange(connection.user_id, connection.exchange_name, credentials)
      } catch (error) {
  ...
  private registerRestoreFailure(
    ...
    const adapter = this.createAdapter(connection.exchange_name)
    if (adapter) {
