...
// Whether a persisted row (bracket pair, trail, settlement) with this
// account_id belongs to `accountId`. Rows without an account are legacy rows
// of the DEFAULT connection: they match a default-connection account only,
// never a labeled one ('acct1/eth'). No account asked → every row matches.
export function rowOnAccount(rowAccountId: string | null | undefined, accountId: string | null | undefined): boolean {
  if (accountId == null) return true
  if (rowAccountId == null) return accountKeyOf(accountId) === undefined
