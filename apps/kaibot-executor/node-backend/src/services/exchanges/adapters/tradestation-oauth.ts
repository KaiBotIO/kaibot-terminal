...
import { lookupTradeStationOrderStatus, resolveTradeStationTif, tsPriceString } from './tradestation-orders.js'
...
export class TradeStationOAuthAdapter extends EventEmitter implements ExchangeAdapter {
  ...
  async getBalances(): Promise<Balance[]> {
    try {
      ...
      return balances.map((bal: any) => ({
        ...
        equity: bal.Equity || 0,
        realizedPnL: bal.RealizedProfitLoss || 0,
        unrealizedPnL: bal.UnrealizedProfitLoss || 0,
        initialMargin: bal.InitialMarginRequirement || 0,
        maintenanceMargin: bal.MaintenanceMarginRequirement || 0,
  ...
  async getPositions(): Promise<Position[]> {
    try {
      const data = await this.call('/v3/brokerage/accounts/positions')
