...
import { TS_TERMINAL, lookupTradeStationOrderStatus, resolveTradeStationTif, tsPriceString } from './tradestation-orders.js'
  ...
  async getBalances(): Promise<Balance[]> {
    try {
      ...
      return balances.map((bal: any) => ({
        ...
        // Margin lives NESTED under BalanceDetail (like the PnL fields above) —
        // the top-level fields don't exist in the v3 payload, so the old
        // mapping read $0 even with an open position (verified raw 2026-08-24:
        // BalanceDetail.InitialMargin 4638 on a live 1-MNQ account).
        initialMargin: num(bal.BalanceDetail?.InitialMargin ?? bal.InitialMargin ?? bal.InitialMarginRequirement),
        maintenanceMargin: num(bal.BalanceDetail?.MaintenanceMargin ?? bal.MaintenanceMargin ?? bal.MaintenanceMarginRequirement),
  ...
  async getPositions(): Promise<Position[]> {
    try {
      ...
      return positions.map((pos: any) => {
        ...
        // Real futures leverage = notional / initial requirement (v3 carries
        // both: MarketValue 58886 / InitialRequirement 4638 ≈ 12.7x on MNQ).
        // Hardcoding 1 made the breathing-room guard treat any follow-up open
        // on a rooted account as full-notional margin — the same class of
        // false reject the flat-account default fixed (2026-08-24).
        const marketValue = num(pos.MarketValue)
        const initialReq = num(pos.InitialRequirement)
        const leverage = initialReq > 0 && marketValue > 0 ? marketValue / initialReq : 1
        return {
