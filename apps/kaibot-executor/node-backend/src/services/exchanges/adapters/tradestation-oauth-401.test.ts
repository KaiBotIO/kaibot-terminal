...
describe('TradeStationOAuthAdapter call() 401 handling', () => {
  it('does NOT re-send an order POST on 401 (refreshes token, then surfaces a clear error)', async () => {
    ...
    installFetch((url, init) => {
      ...
      // The order POST: always 401 here. NB: placeOrder posts to
      // /v3/orderexecution/orders (the mock used to watch /v3/brokerage/orders
      // — the cancel/read route — so the POST fell through to the 200
      // catch-all and the 401 path was never exercised).
      if (url.includes('/v3/orderexecution/orders') && (init?.method ?? 'GET') === 'POST') {
        orderPosts.push({ url })
