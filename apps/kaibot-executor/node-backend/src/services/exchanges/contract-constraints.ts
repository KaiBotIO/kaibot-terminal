...
// TradeStation quantities are whole contracts/shares for every instrument, and
// symbols arrive both as roots (MES) and dated contracts (MESU26) — a venue
// rule beats per-symbol entries. Without it the fallback {0,0} skips integer
// rounding entirely and a factor of 0.5 sends "Quantity: 0.5" to the broker.
const TS_CONSTRAINTS: ContractConstraints = { minSize: 1, stepSize: 1 }

export function getContractConstraints(exchange: string, symbol: string): ContractConstraints {
  if (exchange.toLowerCase() === 'tradestation') return TS_CONSTRAINTS
  const key = keyOf(exchange, symbol)
