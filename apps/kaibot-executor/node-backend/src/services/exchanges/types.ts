...
export interface OrderQueryContext {
  ...
  symbol?: string;
  category?: string;
  // Earliest moment the order can date from (epoch ms). A history lookup
  // (TradeStation /historicalorders) starts its window here instead of at
  // the venue's default of a few days back.
  sinceMs?: number;
}
