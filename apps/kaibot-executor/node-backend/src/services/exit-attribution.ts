...
/**
 * One order's fee spread over the fills it produced, pro rata by quantity. A
 * close that lands on three entries books three exit fills; the venue charged
 * once, so the pieces must add back up to the order's fee.
 */
export function splitFee(
  fee: { commission: number; feeNative?: number | null; feeCurrency?: string | null },
  qtys: number[],
): FeeShare[] {
  ...
  return qtys.map((q) => {
    ...
    return {
      commission: (fee.commission || 0) * share,
      feeNative: fee.feeNative != null ? fee.feeNative * share : null,
...
export interface VenueExit {
  ...
  price: number | null
  orderId?: string | null
  /** USD fee the venue charged on this close; split pro rata over the holders. */
...
export function attributeVenueExit(db: ExitAttributionDb, exit: VenueExit): ExitAllocation[] {
  ...
  const fees = splitFee(
    { commission: exit.commission ?? 0, feeNative: exit.feeNative, feeCurrency: exit.feeCurrency },
    plan.map((a) => a.qty),
  )
