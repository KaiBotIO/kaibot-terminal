...
export async function backfillCommissions(deps: BackfillDeps, opts: BackfillOptions = {}): Promise<BackfillReport> {
  ...
  for (const group of groups.values()) {
    ...
    const ctx: OrderQueryContext = { symbol: head.symbol, ...(head.account_id ? { accountId: head.account_id } : {}) }
    try {
      ...
      if (!fee) {
        ...
        for (const f of group) push({ ...base(f), after: null, feeNative: null, feeCurrency: null, status: miss })
        continue
      }
      apply(group, fee)
    } catch (err: any) {
