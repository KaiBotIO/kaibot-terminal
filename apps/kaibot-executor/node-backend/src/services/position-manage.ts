...
export function findAdoptableStopSeed(
  ...
  const pairs = db.listBracketPairs().filter((p) => p.exchange === exchange && p.sl_order_id)
  ...
  const retired = db.findLatestRetiredTrailForSymbol(exchange, symbol)
  ...
  const manualIds = new Set(db.listManualEntrySignalIds(exchange, symbol))
  ...
  //    same-direction entries: an opposing entry's stop protects nothing here.
