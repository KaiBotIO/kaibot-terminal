...
interface ServerBot {
  ...
  // Per-market run status, same key. 'phasing_out' is projected onto the local
  // config so the edge clips entries while the server finishes the ride.
  runStatusByMarket?: Record<string, string> | null;
}

// Local status after a sync. A locally-set stop/pause always wins (a re-sync
// must never restart what the user stopped); the server's phase-out and its
// resume are the only server-driven transitions, and they only move between
// 'running' and 'phasing_out'.
export function projectConfigStatus(
  local: 'running' | 'paused' | 'stopped' | 'phasing_out' | undefined,
  server: string | undefined,
): 'running' | 'paused' | 'stopped' | 'phasing_out' {
  if (local === undefined) return server === 'phasing_out' ? 'phasing_out' : 'running';
  if (local === 'running' && server === 'phasing_out') return 'phasing_out';
  if (local === 'phasing_out' && server === 'running') return 'running';
  return local;
}
  ...
  async sync(): Promise<number | null> {
    ...
    for (const bot of bots) {
      ...
      for (const m of markets) {
        ...
        const serverStatus = bot.runStatusByMarket?.[`${m.exchange}|${m.symbol}|${m.timeframe}`];
        this.db.upsertBotConfig({
          ...
          status: projectConfigStatus(existing?.status, serverStatus),
