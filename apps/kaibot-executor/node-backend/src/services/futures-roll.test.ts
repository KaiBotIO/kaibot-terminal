...
function fakeDb() {
  ...
  return {
    ...
    applyServerExitUpdate(positionId: string, patch: any) {
      ...
      if (s) {
        Object.assign(s, { last_exit_seq: patch.exitSeq, current_stop: patch.currentStop ?? null, sl_order_id: patch.slOrderId ?? null })
        if (patch.engineStop !== undefined) s.engine_stop = patch.engineStop
      }
    },
    updateServerExitStopFloor(positionId: string, fields: any) {
      const s = exitStates.find((x) => x.position_id === positionId)
      if (s && fields.manualStop !== undefined) s.manual_stop = fields.manualStop
      if (s && fields.trailingLock !== undefined) s.trailing_lock = fields.trailingLock ? 1 : 0
    },
...
describe('handleFuturesRollNotice', () => {
  ...
  it('auto-roll: the stop floor rolls along and keeps composing on the new contract', async () => {
    const { deps, adapter, db } = await setup({ autoRoll: true, armed: true })
    // Floor above the bot stop (7660): venue stop at the floor.
    Object.assign(db.exitStates[0], { manual_stop: 7700, trailing_lock: 0, engine_stop: 7660, current_stop: 7700 })
    const out = await handleFuturesRollNotice(notice(), deps)
    expect(out!.rolled[0]!.result.status).toBe('rolled')
    const floorRolled = rolledStopPrice(7700, RATIO, 'long', 'MESZ26')
    const engineRolled = rolledStopPrice(7660, RATIO, 'long', 'MESZ26')
    const stops = [...(adapter as any).orders.values()].filter((o: any) => o.order.orderType === 'stop' && o.status === 'working')
    expect(stops).toHaveLength(1)
    expect(stops[0].order.stopPrice).toBe(floorRolled)
    const state = db.exitStates[0]
    expect(state.manual_stop).toBe(floorRolled)
    expect(state.engine_stop).toBe(engineRolled)
    expect(state.current_stop).toBe(floorRolled)
  })
