...
import { armedView, type ArmedView } from './synthetic-guard.js';
import { syntheticBasisUsd, type SyntheticBasisKind } from './synthetic-sizing.js';
...
// Synthetic USD block: its own entity, NEVER merged into `positions`. The
// planned floor is conditional (armed, nothing minted yet); the realized floor
// is holdings × fill of a minted arm-cycle short, or the plain target of a
// manually minted one.
export interface PortfolioSyntheticRow {
  id: string;
  exchange: string;
  accountId: string;
  symbol: string;
  ...
  shortSize: number;
  armed: ArmedView;
  isFactorBasis: boolean;
  // Non-null when this row sizes signals (flagged): value + phase.
  sizingBasis: { usd: number; kind: SyntheticBasisKind } | null;
}

export interface PortfolioSynthetic {
  protectedPlannedUsd: number;
  protectedRealizedUsd: number;
  positions: PortfolioSyntheticRow[];
}
...
export interface Portfolio {
  ...
  positions: Array<Position & { exchange: string; accountKey?: string | null }>;
  syntheticUsd: PortfolioSynthetic;
}

export function assembleSyntheticBlock(db: KaiBotDatabase): PortfolioSynthetic {
  // Defensive on test doubles / a failing read: the synthetic block must never
  // take the whole snapshot down (the companion push depends on it).
  let rows: SyntheticUsdPositionRow[] = [];
  try {
    const list = (db as Partial<KaiBotDatabase>).listSyntheticUsdPositions;
    rows = typeof list === 'function' ? list.call(db, false) : [];
  } catch (err) {
    db.log?.('warn', 'system', 'Snapshot: synthetic block failed', { error: errMsg(err) });
    rows = [];
  }
  let planned = 0;
  let realized = 0;
  const positions: PortfolioSyntheticRow[] = rows.map((r) => {
    const armed = armedView(r);
    if (r.status === 'armed') planned += armed.protectedUsd ?? 0;
    else realized += armed.inCycle ? armed.protectedUsd ?? r.target_usd : r.target_usd;
    return {
      id: r.id,
      exchange: r.exchange,
      accountId: r.account_id,
      symbol: r.symbol,
      status: r.status as 'open' | 'armed',
      targetUsd: r.target_usd,
      shortSize: r.short_size,
      armed,
      isFactorBasis: r.is_factor_basis === 1,
      sizingBasis: r.is_factor_basis === 1 ? syntheticBasisUsd(r) : null,
    };
  });
  return { protectedPlannedUsd: planned, protectedRealizedUsd: realized, positions };
}
...
export async function assemblePortfolio(
  ...
  const positions: Array<Position & { exchange: string; accountKey?: string | null }> = [];
  ...
  for (const session of sessions) {
    ...
    for (const p of livePositions) {
      if (Math.abs(p.size) > 0) {
        positions.push({ ...p, exchange: session.exchangeName, accountKey: session.accountKey ?? null });
      }
    }
  }
  ...
  return {
    ...
    allocation,
    positions,
    syntheticUsd: assembleSyntheticBlock(db),
  };
}
