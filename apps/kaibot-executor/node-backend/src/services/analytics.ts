...
export interface ClosedRow {
  ...
  /** Venue the position sat on; labels the per-account rows. */
  exchange?: string | null
  realizedPnl: number // net realized P&L for the trade (after commissions)
  /** Realized P&L before fees; absent on rows that predate fee tracking. */
  grossPnl?: number
  /** Fees the venue charged on the closed quantity, USD. */
  commission?: number
...
export interface AnalyticsSummary {
  ...
  netPnl: number
  // netPnl = grossPnl - commission. Both zero when no trades.
  grossPnl: number
  commission: number
}
...
export function buildStrategyScorecards(closed: ClosedRow[]): StrategyScorecard[] {
  return groupScorecards(closed, strategyKeyOf, (t) => ({
    strategy: t.strategy?.trim() || UNATTRIBUTED_STRATEGY,
    signalBotId: t.signalBotId ?? null,
  })).sort((a, b) => {
...
/** One broker account's scorecard: the same metrics, over the trades it held. */
export interface AccountScorecard {
  /** Stable row id: the account id, or UNATTRIBUTED_ACCOUNT. */
  key: string
  /** "deribit · acct1/eth", or the bare account when the venue is unknown. */
  account: string
  exchange: string | null
  accountId: string | null
  summary: AnalyticsSummary
...
// Executions from before account tracking (migration 032) carry no account.
export const UNATTRIBUTED_ACCOUNT = 'No account'

export function accountRowKeyOf(r: Pick<ClosedRow, 'accountId'>): string {
  return r.accountId?.trim() || UNATTRIBUTED_ACCOUNT
}

/**
 * Per-account breakdown of the same closed trades, same conventions as the
 * per-strategy table. Sorted by net P&L, best first; rows without an account
 * sort last.
 */
export function buildAccountScorecards(closed: ClosedRow[]): AccountScorecard[] {
  return groupScorecards(closed, accountRowKeyOf, (t) => {
    const accountId = t.accountId?.trim() || null
    const exchange = t.exchange?.trim() || null
    const account = accountId == null ? UNATTRIBUTED_ACCOUNT : exchange ? `${exchange} · ${accountId}` : accountId
    return { account, exchange, accountId }
  }).sort((a, b) => {
    const aLast = a.accountId == null
    const bLast = b.accountId == null
    if (aLast !== bLast) return aLast ? 1 : -1
    return b.summary.netPnl - a.summary.netPnl
  })
}

// Shared grouping: every row runs the SAME buildSummary/buildCumulativePnl, so
// a group's win rate, profit factor and drawdown follow the panel's conventions.
function groupScorecards<L extends object>(
  closed: ClosedRow[],
  keyOf: (t: ClosedRow) => string,
  labelOf: (t: ClosedRow) => L,
): Array<L & { key: string; summary: AnalyticsSummary; curve: CumulativePnlPoint[]; firstTradeAt: number; lastTradeAt: number }> {
  const groups = new Map<string, { label: L; rows: ClosedRow[] }>()
  for (const t of closed) {
    const key = keyOf(t)
    const cur = groups.get(key) ?? { label: labelOf(t), rows: [] }
    cur.rows.push(t)
    groups.set(key, cur)
  }
  const out = []
  for (const [key, g] of groups) {
    ...
    // per-group slice preserves that order, which the drawdown pass needs.
    out.push({
      ...g.label,
      key,
      summary: buildSummary(g.rows),
