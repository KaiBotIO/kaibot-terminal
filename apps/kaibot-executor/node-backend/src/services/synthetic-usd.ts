...
export function createSyntheticUsdService(
  ...
  async function adapterFor(exchange: string) {
  ...
  async function placeAndSettle(
    ...
    qty: number,
    reduceOnly: boolean,
  ): Promise<{ orderId: string; filledQty: number }> {
    const adapter = await adapterFor(exchange)
    return withOrderLock(exchange, async () => {
      ...
      return { orderId: result.orderId, filledQty: settled.filledQuantity ?? qty }
    })
  }
  ...
  async function mint(input: {
    ...
    const existing = db.getOpenSyntheticUsdPosition(input.exchange, input.accountId, input.symbol)
    if (existing) throw new Error('an open synthetic USD position already exists for this market')
    ...
    const sizing = computeSizing(input.targetUsd, basis, stepSize, leverageCap)
    ...
    db.insertSyntheticUsdPosition({
      ...
      short_size: sizing.shortContracts,
    ...
    db.insertSyntheticUsdMutation({
      ...
      short_size_before: 0,
      short_size_after: sizing.shortContracts,
      order_id: order.orderId,
      order_side: 'sell',
      order_qty: sizing.shortContracts,
    })
  ...
  async function scale(
    ...
    const plan = planMutation(pos.short_size, sizing.targetUsd, stepSize)
  ...
  return { mint, scale, close, setFactorBasis, setAutoRebalance }
}
