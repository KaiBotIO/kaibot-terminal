...
export interface ManualTradeDeps {
  userId?: string
  registerBracket?: (exchange: string, signalId: string, slOrderId?: string, tpOrderIds?: string[]) => void
...
export function createManualTradeService(
  ...
  async function place(input: ManualOrderInput): Promise<ManualOrderResult> {
    ...
    const accountId = await resolveAccountId(adapter, input.accountId)
    ...
    const quantity = await resolveOrderQuantity(
      adapter,
      input,
      orderType,
  ...
  async function close(input: ManualCloseInput): Promise<ManualCloseResult> {
    ...
    const pos = positions.find((p) => p.symbol === input.symbol && Math.abs(p.size) > 0)
    if (!pos) throw new Error(`no open position for ${input.symbol}`)
    ...
    return withOrderLock(input.exchange, async () => {
      ...
      const manualIds = db.listManualEntrySignalIds(input.exchange, input.symbol)
      ...
      // deactivate the row, so a resting stop can't orphan/double-exit.
      if (fraction === 1) {
        for (const trail of db.findActiveTrailsForSymbol(input.exchange, input.symbol)) {
