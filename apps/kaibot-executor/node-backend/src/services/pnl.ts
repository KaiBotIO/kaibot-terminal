...
export function computeSignalPnl(
  ...
  // Inverse perps (Deribit BTC/ETH-PERPETUAL, Bybit *USD): qty is USD notional
  // and the contract pays out in coin — USD P&L is qty × (exit − entry) / entry,
  // not (exit − entry) × qty (that was off by a factor of the entry price,
  // 2026-09-04 crypto-top2 §4.3). Linear contracts keep the plain formula.
  const inverse = isInverseContract(exec.exchange ?? null, exec.symbol)
  const usdPnl = (from: number, to: number, qty: number) =>
    inverse ? (from > 0 ? (sign * (from - to) * qty) / from : 0) : sign * (from - to) * qty * mult
  ...
  const realizedPnl = entryAvg != null && exitAvg != null ? usdPnl(entryAvg, exitAvg, qtyClosed) : 0
  ...
  if (exec.status === 'open' && qtyRemaining > 0 && entryAvg != null && lastPrice != null) {
    unrealizedPnl = usdPnl(entryAvg, lastPrice, qtyRemaining)
  }
