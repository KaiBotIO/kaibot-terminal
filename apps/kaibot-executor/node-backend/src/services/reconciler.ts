...
import { accountKeyOf, sessionsForExchange } from './exchanges/account-scope.js'
...
export class Reconciler {
  ...
  private async observeExchange(exchangeName: string, summary: ReconcileSummary): Promise<void> {
    ...
    // One pass PER CONNECTION. Two Deribit accounts on one venue each hold
    // their own book: the executions on 'acct1/eth' are held by the acct1
    // connection, the default connection knows nothing about them. Comparing
    // every execution against one connection's positions read "expected 981,
    // broker 0" on every tick (2026-09-23 → 25: 242 false ETH-PERPETUAL alerts).
    const sessions = (await sessionsForExchange(exchangeManager, 'default', exchangeName)).filter(
      (s) => s.status === 'connected',
    )
    if (sessions.length === 0) return

    const allExecs = db.listOpenExecutionsForExchange(exchangeName)
    const allSymbolRows = (
      typeof (db as any).listExecutionAccountSymbols === 'function'
        ? (db as any).listExecutionAccountSymbols(exchangeName)
        : db.listExecutionSymbols(exchangeName).map((r) => ({ account_id: null, symbol: r.symbol }))
    ) as Array<{ account_id: string | null; symbol: string }>

    const now = Date.now()
    for (const session of sessions) {
      // Legacy rows without an account belong to the default connection.
      const connection: string | undefined = (session as { accountKey?: string }).accountKey ?? undefined
      const onThisConnection = (accountId: string | null | undefined) => accountKeyOf(accountId) === connection

      const symbols = [...new Set(allSymbolRows.filter((r) => onThisConnection(r.account_id)).map((r) => r.symbol))]
      if (symbols.length === 0) continue

      const expected = new Map<string, number>()
      for (const e of allExecs) {
        if (!onThisConnection((e as { account_id?: string | null }).account_id)) continue
      ...
      const label = connection ? `${exchangeName} (${connection})` : exchangeName
