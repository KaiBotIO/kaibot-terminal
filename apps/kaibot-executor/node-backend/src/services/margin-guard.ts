...
// Futures brokers margin a contract at a fraction of notional (1 MNQ ≈ $4-6k
// initial on ~$59k notional). The full-notional default above rejects EVERY
// first entry whose notional exceeds account cash — the 2026-08-24 E2E saw
// 1 MNQ refused on a $12k account (orderMargin $58.9k). 10x ≈ a 10%
// initial-margin assumption, still stricter than the ~12-15x brokers grant.
export const FUTURES_VENUES: ReadonlySet<string> = new Set([
  'tradestation',
  'interactivebrokers',
])
export const FUTURES_DEFAULT_LEVERAGE =
  Number(process.env.BREATHING_ROOM_FUTURES_LEVERAGE) || 10

// Venue-aware fallback for the leverage chain (metadata → same-root position →
// heaviest position → this).
export function defaultLeverageFor(exchangeName: string): number {
  return FUTURES_VENUES.has(exchangeName.toLowerCase())
