...
export interface BotSubscriptionRoute {
  id: string
  account_id: string | null
  account_key: string | null
}

// One bot, several subscriptions, each routed to its own connection
// (account_key) and optionally pinned to one account (account_id). The
// adoption targets a venue position on a known account, so the subscription
// is the one whose route covers that account, never "the bot's first sub".
export function pickSubscriptionForAccount<T extends BotSubscriptionRoute>(subs: T[], accountId?: string): T {
  const routeOf = (s: BotSubscriptionRoute) => s.account_key ?? 'default'
  if (!accountId) {
    if (subs.length === 1) return subs[0]!
    throw new Error(
      `this bot has ${subs.length} subscriptions (connections: ${subs.map(routeOf).join(', ')}); pass accountId`,
    )
  }
  const key = accountKeyOf(accountId)
  const onConnection = subs.filter((s) => (s.account_key ?? undefined) === key)
  const match = onConnection.find((s) => !s.account_id || s.account_id === accountId) ?? null
  if (match) return match
  if (onConnection.length > 0) {
    throw new Error(
      `this bot's subscription on the ${key ?? 'default'} connection routes to account ${onConnection
        .map((s) => s.account_id)
        .join(', ')}, not ${accountId}`,
    )
  }
  throw new Error(
    `no active subscription of this bot routes through the ${key ?? 'default'} connection (subscriptions: ${subs
      .map(routeOf)
      .join(', ')})`,
  )
}
...
export function createAdoptPositionService(
  ...
  function resolveLineage(sig: SignalRow, req: { exchange: string; symbol: string; accountId?: string }) {
    ...
    const subs = db.listActiveSubscriptionsForBot(botId)
    if (subs.length === 0) throw new Error('no active subscription for this bot')
    const sub = pickSubscriptionForAccount(subs, req.accountId)
  ...
  function lineageStop(signalId: string): string | null {
    const pair = db.getBracketPair(signalId)
  ...
  async function adopt(req: AdoptRequest): Promise<AdoptResult> {
    ...
    return withOrderLock(req.exchange, async () => {
      ...
      if (slOrderId) {
        ...
        const seed = findAdoptableStopSeed(db, req.exchange, pos.symbol, { direction, mark }, accountId)
        if (seed) {
