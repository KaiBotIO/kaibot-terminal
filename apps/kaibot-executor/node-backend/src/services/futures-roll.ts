...
function lineageProtections(db: KaiBotDatabase, exchange: string, ids: string[]): LineageProtection[] {
  ...
  for (const id of ids) {
    ...
    out.push({
      ...
      stop: state?.current_stop ?? entryRow?.stop_loss ?? null,
...
async function rollHeld(
  ...
  for (const p of protections) {
    ...
    const stop = rolledStopPrice(p.stop, notice.ratio, p.direction, notice.to)
    let orderId: string | null = null
    try {
      orderId = await rearmStop(deps, adapter, p, notice.exchange, notice.to, held.accountId, stop, p.openQty)
    } catch (err: any) {
