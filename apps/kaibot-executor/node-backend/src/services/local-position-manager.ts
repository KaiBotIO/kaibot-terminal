...
export class LocalPositionManager {
  ...
  async tick(): Promise<void> {
    ...
    if (trails.length === 0 && managed.length === 0) return
    ...
    for (const [exchangeName, rows] of byExchange) {
      ...
      for (const row of managedByExchange.get(exchangeName) ?? []) {
      ...
      // 2) Trail rows (the stop owners).
      for (const row of rows) {
        if (closedSymbols.has(row.symbol.toLowerCase())) continue
        const live = positions.find(
          (p) => p.symbol.toLowerCase() === row.symbol.toLowerCase() && Math.abs(p.size) > 0,
        )
        if (!live) {
  ...
  private async advanceTrail(
    ...
    if (effective == null || !stopsDiffer(effective, row.current_stop)) {
      persistMarks()
      return
    }
    ...
    await withOrderLock(exchangeName, async () => {
      ...
      const live = positions.find(
        (p: any) => p.symbol.toLowerCase() === row.symbol.toLowerCase() && Math.abs(p.size) > 0,
      )
      if (!live) {
        this.db.deactivateLocalTrail(row.signal_id)
        return
