...
export interface AccountOutcome {
  ...
  // Server exit updates: what rests at the venue for THIS lineage after the
  // attempt (null = nothing rests). Present only when the lineage reported it.
...
export function outcomeFor(input: {
  ...
  if (last) {
    return {
      ...
      ...('venueStop' in last ? { venueStop: last.venueStop ?? null } : {}),
    }
  }
  // A lineage that threw before acking reported nothing about its stop; an
  // exit update that threw after cancelling would have acked (refuse) first.
