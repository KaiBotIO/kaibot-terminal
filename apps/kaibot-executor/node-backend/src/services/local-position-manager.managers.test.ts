...
describe('LocalPositionManager — edge manager engine', () => {
  ...
  it('manager candidate composes with the manual stop rule (manual participates, engine improves)', async () => {
    ...
    // BE candidate 100.1 < manual 101 → the manual level wins. No order was
    // resting yet (attach adopted nothing), so the composed 101 stop must be
    // PLACED on the venue — before the vangnet fix this stayed a DB-only stop.
    const stops = stack.placed.filter((o) => o.orderType === 'stop')
    expect(stops).toHaveLength(1)
    expect(stops[0].stopPrice).toBeCloseTo(101, 9)
    expect(db.getLocalTrail(KEY)!.current_stop).toBeCloseTo(101, 9)

    // Second tick: the stop now rests on the venue → no re-placement.
    positions[0].markPrice = 102.5
    await mgr.tick()
    await orderLockIdle()
    expect(stack.placed.filter((o) => o.orderType === 'stop')).toHaveLength(1)
  })
