...
// support). 0.4.0 = exit-as-signal-update (server exit authority registration
// + `update` handling, migration 031). The server gates delivery on these
// floors — a build WITH a capability but a LOWER stamp gets its signals held
// in the replay queue forever (armed ascender entries never delivered while
// the go-live binary said 0.1.2).
export const EXECUTOR_VERSION = process.env.KAIBOT_EXECUTOR_VERSION ?? '0.4.10'
