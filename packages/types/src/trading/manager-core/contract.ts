...
export type ManagerAction =
  ...
  // cash) / `size` (absolute units); `sizeFraction` wins when both are set.
  // `liveSize` is the FACTOR the live runner emits on the add-signal.
  // `limitPrice` (optional): the rung/limit level this scale-in targets.
  | {
      type: 'scale_in';
      size?: number;
      sizeFraction?: number;
